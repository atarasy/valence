...
- **A mandate this host already holds is never replaced**, for the reason an offer is not: loosening a mandate needs its co-signers (§16.1), and an import carries no signatures. **Receipts, the recovery log, permissions and queries are added to**, by `ref` or `id`, and never replaced, so a second import cannot erase what the host recorded.
