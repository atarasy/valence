...
An implementation MUST refuse the whole import with `422` when any of these fails, `409` for an offer it already holds, and MUST refuse an export whose `format` it does not recognise with `400`. An implementation MAY read `valence-node/4`, whose collections carry no `missing` and no `missing_notes`, as a collection that recorded nothing missing. Added on 2026-09-09: the reference engine's import verified none of it, and an adversarial pass planted a forged edge that made a product unofferable to a household that had never seen it.
...
**A set that arrives by a move with no confirmation recorded is refused the same way.** Question 50, decided 2026-09-15. §14 makes the register part of the export, and an offer it does not name is, to the receiving host, a set with no confirmation recorded. Refusing costs that household its cooling window on that set. **The alternative considered and refused** was to tell a collection by its row instead of by the register, which keeps the window and lets a set taken back be decided again with the confirmation it arrived without, by anyone who saw the signature on the way (§10.5). A window lost on a move from a host that dropped the register is the smaller loss than a withdrawal anyone can reverse.
