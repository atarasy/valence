...
**Question 57, decided 2026-09-18 and rebuilt 2026-09-19: money moves at the host that holds its reserve.** An implementation MUST leave behind an offer whose money has not finished moving, which is one at `drafted`, `presented` or `decided`, or a physical box whose collection has not happened or whose statement is still owed, together with every row of the body that names it, and MUST answer with the identifiers it left behind; it MUST NOT refuse the move for them. It MUST refuse a candidate that arrives with no verdict. Measured on the reference, an offer arriving at `presented` was taken with a `201` and **no reservation on the receiving ledger**; §6.4 makes the reserve the upper bound of what an offer may settle at, so such an offer's upper bound was one that ledger had never seen, and what stopped it charging was the ledger refusing `no_reservation`, which another adapter may not.
