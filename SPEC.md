...
**Why this is here rather than in the merchant-side platform.** The duty is the seller's, and the surface is the person's agent's. A specification that left the join to each deployment would leave the person's agent free to decide what a seller's notice says, which is a party that is not the seller speaking as the seller: clause 11 makes a curator that signs sign as the merchant's **disclosed** agent, and clause 45 makes the intermediate layer an agent rather than a reseller. **The hub is a contractor for rendering and never an agent for composing**, and clause 54 is satisfied because a party that renders a statutory notice is a party to no transaction.
