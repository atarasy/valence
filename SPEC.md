...
**A mandate with no signed history here starts at version 1 unless what is submitted is the claim itself.** For a day this section let the household state the version wherever a claim was held, and a third refutation pass measured what that reopened: a raw signature is not bound to a host, so any looser version the household had ever signed could be recorded at a host it moved to, over its own tighter claim. **The cost of version 1** is that a stranger's claim makes the household start its numbering again, which costs a number and not a protection. **An implementation MUST NOT keep a claim at a version no household reaches**, because signing it would leave no room to follow and freeze the mandate, and **a version MUST have room to be followed**: a version at or above 2^53 does not increase when one is added to it, so the version stops rising and an old signature replays onto a new record, which is the one thing §16.1 puts the version in the bytes to prevent.
