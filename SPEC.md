...
`amount` is the line's own, `unit_price × quantity`, or 0 for a gift (§6.2). **A line the collection recorded `missing` is in it as `<candidate>:lost:0:`**, because the household is shown it and may contest it (question 46); a candidate that became `lost` at the deadline is not in it, since no collection said anything about the household's home. A household confirms the statement as proposed, or marks consumed or missing lines `disputed` and signs the rest; it may not add, remove or revalue a line, and the two shapes of §10.5 apply, a signature over these bytes or an assertion whose challenge is their SHA-256, by the key registered for the offer's mandate.
...
**The mandate an offer reads is the offer's household's.** Question 54, decided 2026-09-16. An offer names a mandate id, and nothing bound the two: an offer read another household's ceilings, cooling window and co-signers through the id alone. An implementation MUST read a mandate for an offer only when the mandate's `household` is the offer's, and a mandate of another household is unknown to that offer, which §16.2 already leaves alone. **What this does not settle** is that a mandate id and a key name are each one name across the whole host and belong to whoever registered them first, so one household can take a name another household meant to use: question 55.
