...
**A mandate that arrived by a move is signed here before it is one** (§14.2, question 56, decided 2026-09-18). What a move carries is a claim, and this section is the route by which a claim becomes a mandate: the household signs the claim's own canonical bytes, at the version the claim carries, with a signature or an assertion made for **this** host. That is what makes a planted claim harmless, and it is why the signatures are not carried instead, which the next paragraph forbids and which an assertion could not satisfy across hosts in any case.
