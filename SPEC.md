...
**The ceiling counts per host** (question 63, decided 2026-09-19, accepted as a cost). A household that moves in the middle of a day can settle past its ceiling across the two hosts, because the settlements a move carries are not counted towards the new host's day, and a ceiling tightened at the new host does not reach a set left at the old one (§14.2). Counting the carried settlements was refused: the import authenticates nobody (§13.2), so anybody who can post one could fill a household's day with settlements that never happened. **That reason does not describe the reference, and this was found the same day.** `POST /households/{id}/settled`, the route an engine reports to the hub through, takes any row from any caller and replaces a row by its offer id, so the same anybody can already fill a household's day, and zero it: the third refutation pass over question 64 wrote a household's two settlements back at 0 and settled past its ceiling, and wrote 999,999,999 and had every settlement refused. **A ceiling is a protection only once the hub authenticates who reports to it**, which is question 41's; until then a hub that accepts settled rows from any party other than its own engine has no ceiling, and a deployment that exposes that route has none (question 67, recorded 2026-09-19 and deferred to question 41).
