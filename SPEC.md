...
**One payer's settlements run one at a time** (question 66, decided 2026-09-19). The ceiling is read, the ledger is told and the day is told, and wherever one of those waits on the network two settlements for one payer both read the day before either is counted: measured with a ledger whose commit is a round trip and with a day read over HTTP, 2,700 settled against a ceiling of 2,000. An implementation MUST serialise the read, the check, the commit and the report for each payer within one process. Two processes settling for one payer remain the non-atomic case the paragraph below names. **A settlement retried after its report to the day failed MUST be reported again**, and the day keeps one row an offer, so the retry cannot count it twice; the reference answered a retry with the settlement and told nobody, and the day stayed short by that amount for the life of the host. A settlement a move carried is never reported, for question 63's reason. **A retry while the hub cannot be reached answers `422 hub_unreachable` on every attempt, for a settlement that is written and whose ledger has moved**, which is the direction this section chose: a presenter reading the refusal retries, and the retry is safe because the day keeps one row an offer. **An implementation MUST know which settlements a move carried to it**, since it reports none of them and lists none as a giver's payment, and MUST record that as the import writes them. **A store written before it recorded that has carried settlements with nothing to say so**: measured by the second refutation pass over question 66, a payment a recipient's import had planted came back into its giver's record on upgrade and the first settle reported it to the giver's day. **Such a store MUST NOT be repaired by guessing, and MUST be rebuilt.** Two passes were written to mark its rows from the one record that looked like evidence, a settlement with no reservation on the ledger, and two refutation passes refuted both: the reference kept its ledger in memory until 2026-09-19 even where the store was on disk, so every settlement of such a store looked carried; one reservation made after that made the pass mark every older settlement the host had made; a host that has only imported has no reservation to go on at all; and a swept or pruned reservation turns a settlement the host made into a carried one. The reference says once that the store predates the marker and marks nothing. The rule for question 66 is proven by the engine's unit tests and not by a conformance probe: the reference's in-memory ledger resolves every wait at once, so two settlements over HTTP never overlap, and a failed report needs a hub that fails on cue.
