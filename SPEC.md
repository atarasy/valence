...
**A missing item carries a note, and the note travels.** A collection naming a candidate `missing` without a note for it MUST be refused with `422 missing_note_required`. The note is stored with the collection and carried in the household's export and the merchant's (§14, §14.1), so the party holding stock risk receives an attributable, per-item record rather than a silent loss. A note keyed to a candidate the collection did not name `missing` is `400 malformed`. The specification does not judge the note.

**The order of the refusals is part of the rule**, so an implementation built from this text names the same refusal as any other: `unknown_candidate`, then `returned_and_consumed` for a candidate in more than one list, then `missing_note_required`, then `candidate_decided`, then `collection_incomplete`, and `already_collected` for any collection after the first.
