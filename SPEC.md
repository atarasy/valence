...
| `lost` | physical only. Not recovered by the recovery deadline, or recorded `missing` by the collection (§11.2). | not charged to the household |
...
**A presenter's name is first-come, and that is not what question 55 closed.** A household's identifier and a mandate's are the key itself since 2026-09-16 (§13.2); a presenter's is a name registered under a key, so whoever registers it first holds it and a presenter that has not yet registered can be named by somebody else. A presenter may take the `key:` shape and nothing here requires it to.
...
**A set that owes nothing settles at nothing, at once** (question 62, decided 2026-09-19). An implementation MUST settle at 0, releasing the reserve, an offer that is `decided` or `expired` with nothing to charge and nothing on a statement, which is no line consumed or lost and every line kept or defaulted a gift (clause 10): at the decision itself where no cooling window (§16.5) stands, and otherwise no later than the household's export. **A physical box is settled so only once it has been collected**, because §11.2 lets the collection overrule the household's `returned`: a first refutation pass measured a box the household called returned in full settling at 0 on that word alone, after which the collection was refused on a settled offer. **So a box the household called returned and nobody collects still waits for its presenter**, as every set did before this question; what closes it is the collection, which is the presenter's to make. A settlement of nothing is not refused on the daily ceiling and does not ask for the giver's (§16.3). **An implementation MUST mark the offer settled before it reports the settlement to the household's day** (§13.2): a second refutation pass measured a failed report leaving the settlement written and the reserve moved while the offer stayed `decided`, which nothing could then withdraw, settle or move. Until this question such a set held its reserve until the presenter settled it at 0, which nothing obliged it to do, and the row above for expiry was not met. Measured by the fifth refutation pass over question 57: a set like that could not move, because its reserve was on the host it was leaving, so the household's record of what it refused did not reach the host it moved to, and a presenter that wanted that record gone had only to leave the zero-settle unmade. **Nothing settles on a timer**, so on the reference an expired offer's reserve is still held until the presenter settles or the household exports, whichever comes first; a set inside a cooling window waits, because it can still be taken back and decided again.
...
**What is signed.** A domain tag, the offer id, the carriage, then one line per kept, defaulted or consumed candidate and per candidate the collection recorded `missing`, in ascending candidate id, UTF-8, `\n` between lines:
...
`amount` is the line's own, `unit_price × quantity`, or 0 for a gift (§6.2). **A line the collection recorded `missing` is in it as `<candidate>:lost:0:`**, because the household is shown it and may contest it (question 46); a candidate that became `lost` at the deadline is not in it, since no collection said anything about the household's home. A household confirms the statement as proposed, or marks consumed or missing lines `disputed` and signs the rest; it may not add, remove or revalue a line, and the two shapes of §10.5 apply, a signature over these bytes or an assertion whose challenge is their SHA-256, by the key registered for the household the offer's mandate names, which is the mandate identifier up to its full stop (§13.2, question 55).
...
- **A disputed line leaves the rail.** It is not charged, `charged` still equals `kept_amount + consumed_amount`, and the line is returned with `disputed: true` and its amount under `disputed_amount`. Only a consumed line, or a line the collection recorded `missing`, can be disputed; `422 not_disputable` otherwise, since a kept line is one the household signed itself. **A disputed missing line moves no money**, because none was charged; it records that the household contests the loss, and the settlement returns it with `disputed: true` and adds nothing to `disputed_amount`.
  ...
  **What a signature over a missing line attests is only that the line was shown.** A household's signature over a statement that carries a `<candidate>:lost:0:` line is not an acknowledgement that the item is missing, nor of any responsibility for it; §3.2 already says a `missing` record founds no claim against the household, and the signature does not create one. An implementation MUST say so on the screen where the household signs, so silence is not read as agreement. The line stays inside the signed bytes, because the household is being shown it and may dispute it; what changes is what the signature is said to mean.
...
### 6.6a A refund that came back is recorded beside its correction

**Decided 2026-09-23.** A refund can fail after it was made. The merchant refunds on its own account, appends a `refund` correction (§6.6), and weeks later the card issuer returns the refund to the merchant, so the household never receives it. The correction is still true, because the merchant did refund and the household's bill was lowered; what changed is that the household is now owed the amount. A payment provider says the same of its own refunds: Stripe documents (docs.stripe.com/refunds, "Handling failed refunds", read 2026-09-23) that a failed refund's amount goes back to the merchant's balance, which can take up to 30 days, and that the merchant must then arrange to refund the customer another way.

So a correction is never withdrawn and a failed refund is never retried here. The merchant appends a second signed record saying the refund came back, and, once it has repaid the household another way, a third saying so:

```
correction_return
  correction     the id of a refund correction on this offer
  offer          the offer whose correction it follows
  merchant       the correction's own merchant, which signs it
  state          returned or repaid
  note           the merchant's own words, shown to the household as written, at most 500 characters
  at             when, never before the correction, and a repayment never before its return
  signature      by the merchant's registered key, over the canonical form
```

The canonical form is `valence-correction-return/1`, then the correction, the offer, the merchant, the state, the note and the time, one to a line, **every free part percent-encoded** as a correction's are. The first line names the form, so these bytes cannot verify as a correction or any other signed object.

`POST /offers/{id}/returns` takes `{correction, merchant, state, note, at, signature}`, the offer being the path's. **The rules are six.**

1. **Only a refund comes back.** A corrected collection moved no money, so there is nothing to return: an implementation MUST refuse `422 not_a_refund` a record naming one, and `404 unknown_correction` one naming no correction on the offer. An offer with no settlement, or one deleted with its household (§14.3), answers `404 not_found`, as the corrections route does.
2. **It is the correction's merchant's.** An implementation MUST refuse `422 not_merchant_of_record` a record whose merchant is not the correction's, and MUST verify the signature against that merchant's registered key, refusing `422 unknown_merchant` where none is registered and `422 bad_signature` where it does not verify.
3. **Nothing comes back before it was sent.** A record dated before its correction, or a repayment dated before its return, is refused `422 return_before_correction`.
4. **A repayment follows a return.** A `repaid` record with no `returned` record held for its correction is refused `409 not_returned`.
5. **At most one of each.** A correction has at most one `returned` and one `repaid` record. An identical retry answers `200` with the record held; the same correction and state with any field different answers `409 return_conflict`, so a retry never becomes a second record.
6. **The shape is checked first.** A state outside the two, a note over 500 characters, an `at` that is not a non-negative safe integer, or any field the record does not name is `400 malformed`.

**The household's receipt gains two keys, and only when there is something to show.** `GET /offers/{id}/corrections` answers as §6.6 says, and, where at least one record is held for the offer, also `returns`, every record oldest first by `at` and then by arrival, and `owed`, the sum of the amounts of the refund corrections that have a `returned` record and no `repaid` one. **Where no record is held both keys are absent, not null and not empty**, because clients decode this receipt's keys exactly and a key that was always present would fail every receipt a client built before this section reads. Neither record changes `net`: the correction already lowered what the household pays, and what it is owed is a debt between it and the merchant, not a change to the bill.

**It moves with the household.** A household's export carries the records as `correction_returns`, a map from an offer's id to a non-empty list, in `valence-node/12` (§14), where the field is required and may be empty. A `valence-node/10` or `/11` export carries none and is still accepted. An import MUST hold each record to the rules above against the corrections the same body carries, MUST refuse `422 unscoped_return` a record whose offer or correction the body does not carry, and MUST NOT add to or remove from the records of an offer the host already holds.

**What it does not do.** It moves no money, in either direction: the refund was the merchant's and so is the repayment, and the platform neither holds the amount nor pays it. **It builds no route by which a household asks for the money**, as §6.6 builds none for a refund; the hub shows the merchant's signed contact from the offer's disclosure (§10a) beside the record so that the two can settle it directly. **And it does not retry the refund**, because a refund can go only to the payment method that paid, which is the one that just failed, and the provider itself says the repayment must be arranged another way.
...
POST   /offers/{id}/returns         a merchant's signed record that a refund came back, or was repaid another way (§6.6a)
...
5. **Sign.** The decided set is signed as an AP2 mandate, and `POST /offers/{id}/decisions` carries the signature beside the decisions (clause 35). What is signed is the set in this canonical shape, so a signature made by one hub verifies at any conforming endpoint:
   ...
   one line per decision in ascending candidate id, UTF-8, `\n` between lines; the signature is over those bytes, base64, by the key registered for the household the offer's `mandate` names, which is that identifier up to its full stop (§13.2, question 55): a mandate names no key of its own, so nobody can hold one by taking its name first. **A signature is verified by the type of the key it is checked against**: ed25519 over the bytes as they are, ECDSA on P-256 (ES256) or RSA (RS256) over their SHA-256. This said ed25519 alone until 2026-09-11, and it refused the key most devices carry: a member who joined through a hub holds a passkey, which is P-256 on almost every phone and laptop, and that member could register a key and then confirm nothing with it and sign no change to their own mandate (§16.1). The same rule reads every signature this specification names. An implementation MUST refuse, with `422`, a decided set with no signature, a signature by another key, or a signature over a set other than the one sent; nothing is written on refusal, on the durable store as well as in memory. **§16.3's record of a decided set's protections has to be written with the decision and not before it**: the reference wrote it before reporting the offer to the household's day, which is the last thing in that route that can fail, and a day source that threw left the record on disk for a set the store still held as `presented`. Measured by the second refutation pass over question 68; the direction was protective, since a stale record can only refuse more, but the rule is the rule. **A decision that loses a race to another decision of the same set is refused `409 already_decided`** (decided 2026-09-20), the same name §10.3c gives a line an earlier decision or a collection has already resolved, because it is the same fact arriving a moment later: an implementation that reads a household's protections at the decision waits, and the lines can be decided during the wait. An implementation MUST NOT invent a second name for it.
   ...
   **So a decided set carries one of two shapes**, and an implementation MUST accept both.
   ...
   |---|---|---|
   | `signature` | a signature over the canonical bytes, base64 | against the key registered for the mandate's household (§13.2), by that key's type |
   | `assertion` | `authenticator_data`, `client_data_json`, `signature`, base64 each | the `challenge` inside the client data equals the base64url SHA-256 of the canonical bytes, and the signature covers `authenticator_data` concatenated with the SHA-256 of `client_data_json`, by the key registered for the mandate's household (§13.2) |
...
7. **A merchant may sign a contact, and the hub shows it beside its return terms.** Question 72, decided 2026-09-22. `contact` is OPTIONAL: `kind` is `email`, `tel` or `url`, and `value` is the merchant's own. Where present, an implementation MUST render it exactly as the merchant signed it, on every surface that already carries the block (requirement 4). **Where the merchant gave none, the key is absent rather than null**, so a client that reads a block's keys exactly, as the reference's iOS and Android clients do, reads a block from before this field unchanged. **This is not a messaging feature.** The hub hands over nothing through it: no household data leaves, no message is composed or sent on the household's behalf, and no refund is initiated. A household that wants to reach the merchant contacts it itself, by whatever the block gives; the hub's part ends at showing it. An implementation MUST NOT add a route that sends anything to a merchant on a household's behalf under this field, and MUST NOT treat its presence as a refund channel.
...
**A first collection MUST resolve every undecided candidate.** `POST /offers/{id}/recovery` takes `returned`, `consumed`, an optional `missing` and, where `missing` names anything, `missing_notes`, an object giving a non-empty note of at most 500 characters for each missing candidate. It MUST refuse, with `422 collection_incomplete`, a first collection that leaves a candidate still `offered` unnamed in all three, and with `422 candidate_decided` one that names a candidate already decided as anything but `returned` (kept by the household, or made `lost` by the deadline). **It MAY name a candidate the household decided `returned`**, as `consumed` or `missing`, and that verdict replaces the household's: a household may say an item is not wanted, but it is the collection that finds whether it was used or is gone, and an overruled line comes to the household on the statement to sign or dispute. Revised the same day by the founder's decisions on question 46.

**A missing item carries a note, and the note travels.** A collection naming a candidate `missing` without a note for it MUST be refused with `422 missing_note_required`. The note is stored with the collection and carried in the household's export and the merchant's (§14, §14.1), so the party holding stock risk receives an attributable, per-item record rather than a silent loss. A note keyed to a candidate the collection did not name `missing` is `400 malformed`. The specification does not judge the note, and a `missing` record founds no claim against the household (§3.2).

**The order of the refusals is part of the rule**, so an implementation built from this text names the same refusal as any other: `unknown_candidate`, then `returned_and_consumed` for a candidate in more than one list or twice in one, then `missing_note_required`, then `already_collected` for any collection after the first, and only for a first collection `candidate_decided`, then `collection_incomplete`. A repeated collection is therefore refused `already_collected` whatever it names, once its body is otherwise well formed.

Why the rule exists, added 2026-09-14 by question 46. The deadline makes an item `lost` only while nothing has been collected, and a second collection is refused, so an item the first collection left out stayed `offered` for good and the offer could neither settle nor be collected again. And a route that found an item gone had no true verdict to record: `consumed` put it on the household's statement and `returned` said goods came back that did not. **`missing` is never billed**, like any `lost`, and it is not a verdict a decision may carry. That alone did not make it safe, and a review pass on 2026-09-14 said so: unsigned and undisputable, it was a merchant's statement about a household's home that the household never saw, and a loss moved to the stock holder on the merchant's word. So it is on the statement, it may be disputed, it founds no claim (§3.2), and it carries a note.

**Every rule of a collection belongs to the implementation's engine and not to its route**, which this section has assumed since question 46 and which §16.3's record broke for a day. The record of what a box its collection decided is held to was made on a path only the reference's own route took, and the in-process call beside it decided a box and recorded nothing: the second refutation pass over question 68 measured it, and named the service behind one of the reference deployments as a caller of that path. An implementation MUST apply this section's rules, and §16.3's and §16.5's records, wherever a collection is recorded, and MUST NOT make one of them a property of one entry point. **The cost is that recording a collection waits on a read**, and it never refuses on one: a line the deadline resolves is `lost`, which §3.2 never bills, so refusing a collection moves the loss to whoever holds the stock for goods the household did use.
...
**A gift is presented only on its giver's signature** (question 64, decided 2026-09-19). An implementation MUST refuse to present a ceremonial offer with `422 gift_unsigned` unless the body carries the giver's `signature` or passkey `assertion` over the gift, with `422 bad_signature` where it does not verify against the key registered under the giver's name, and with `422 unsigned` where no key is registered under it, and MUST reserve nothing before it verifies. **The giver is a household identifier** (§13.2): an implementation MUST refuse a ceremonial offer whose `giver` is not one with `422 name_is_not_the_key`. The giver is the name its signature is checked against, and a name that is not a key was first-come at registration, so the third refutation pass over this question measured a presenter registering its own key under `grandmother-tanaka`, signing, presenting and settling 2,100 against that name (question 58). The bytes are, one to a line, `valence.gift.2`, the host (the relying party it asserts for, as §16.1's mandate form names it), the offer's id, the giver, the recipient household, the presenter, the band's minimum and maximum, the most the offer can come to (§6.4's upper bound, which leaves out what a maker gave) and the expiry, **with the host and the three names percent-encoded**, so that a name holding the separator cannot make two sets of terms one set of bytes. `valence.gift.1` named no host, and what kept a signature from being replayed at another host was only that the host mints the offer id; an implementation that let a presenter choose one would have inherited question 58 in full. A host serves the terms, its own name among them, at `GET /offers/{id}/gift` with the passkey challenge beside them, and a giver's device recomputes them from the offer rather than trusting them. Until this question the giver was whatever household the presenter wrote: measured by a refutation pass, a household named as giver was charged 1500 without any act of its own, and after question 60 its own purchases were then refused on its daily ceiling. An offer that is not ceremonial MUST NOT carry the field.

**A ceremonial offer is digital.** An implementation MUST refuse `purpose: ceremonial` with `binding: physical`, with `422 ceremonial_is_digital`. Added 2026-09-12, after a refutation pass found that the combination breaks §6.5's premise: clause 25 makes the giver the party charged and §6.5 makes the household's signature over the settlement statement the application for a consumed line, so a physical ceremonial box charges a party that took no act at settlement while the party that acted pays nothing. The same offer also reported what settled against the recipient's daily ceiling (§16.3) while the giver paid, which question 60 closed for every ceremonial offer.
...
13. enforces the person's thresholds where each is enforced, and names the one that refused (§16.3 to §16.6), applies no cooling window to a settlement whose application is the household's signature over a statement, and refuses to withdraw a set with no confirmation recorded, whether it reached its state through a collection or arrived by a move without one (§16.5, §14)
...
**A hub MUST take the two reports only from its engine** (question 67, decided 2026-09-22). `POST /households/{id}/offers` and `POST /households/{id}/settled` write the person's own record and the day a daily ceiling is read against, so a hub that took them from any caller had no ceiling: anybody could fill a household's day, or zero it. The engine proves itself with a service credential the deployment gives both processes, sent as `Authorization: Bearer`; the hub compares it in constant time and refuses anything else with `401 unauthenticated_report` before reading the body. **A hub given no credential takes no report at all**, which is right where one process presents both roles: its engine writes the ledger in-process and nothing else may. The read, `GET /households/{id}/settled`, is unchanged. **What this does not do**: it authenticates the engine, not a household, so it closes the report routes and nothing else; the import is still unauthenticated (§14.2), which is why question 63's reason for not counting carried settlements stands. The reference reads the credential from `VALENCE_ENGINE_REPORT_TOKEN`, refuses one shorter than 32 characters, and refuses to start an engine pointed at a remote hub without one.
...
**A name a signature is checked against is the key.** Question 55, decided 2026-09-16.

A household's identifier is `key:` followed by the base64url SHA-256 of its public key in SubjectPublicKeyInfo DER form, which is 47 characters whatever the key's type. `/_identities` MUST refuse with `422 name_is_not_the_key` a registration whose name begins `key:` and whose key does not hash to it, whoever sends it. A name can therefore be claimed by nobody but the holder of the key, and first-come has nothing left to decide.

A mandate's identifier is its household's identifier, a full stop, and a label of one to 64 characters the household chooses from `A-Za-z0-9_-`. **The key a decided set, a settlement statement and a mandate version are checked against is that household's own**, so a mandate names no key of its own and an implementation MUST NOT look one up for it. An implementation MUST refuse with `422 name_is_not_the_key` a mandate whose `household` is not a `key:` name, a mandate whose `id` is not that household's identifier followed by a full stop and a label, and an offer whose `household` or `mandate` is not of those shapes. **A co-signer is named by the key it signs with too**, and an implementation MUST refuse a mandate, recorded or arriving, whose `co_signers` carries a name that is not a `key:` one. That was not part of the decision and was found by a refutation pass the same day, measured: a stranger registers `mum`, a household records its first version naming `mum`, which needs nobody else's signature, and every loosening after that is the stranger's to sign. Clause 47 rests on who the named people are, so the question reached one namespace further than it was asked about. **The shape has to be refused where an offer is created and not only where a mandate is recorded**, because a decided set is verified against whatever key the offer's mandate names, so an offer naming a mandate outside this shape is one a presenter can confirm for the household.

**Why not an authority.** Clause 2 puts the root of identity outside this system and says that nothing here issues one, so a host that handed out names would be issuing identities. Assigning a scarce name has only two other forms: first-come, which is what this closes, and a name that commits to a key, which is this.

**Why a hash rather than a name that carries the key.** §10.5's verifier reaches ed25519, ECDSA on P-256 and P-384, and RSA. A `did:key` carrying an RSA key is about 380 characters in every path and every export, and one carrying the curves alone silently excludes the RS256 authenticators some platforms issue. **What the hash costs is that `/_identities` goes on carrying key material** rather than becoming attestation alone: the name says which key it is, and the key is still handed over once. A host may accept that key from anybody, because the name proves it.

**What this does not close, stated plainly.** The import authenticates nobody (§14.2), so a mandate row can still be planted under a household's identifier and §14.2 keeps the row the host already holds: that is question 56, and this does not reach it. A merchant's, a presenter's and a lineage edge's giver's name stay first-come under §5.2's limit (a ceremonial offer's giver is a key since question 58, §12), **and a giver is a household on some host, so a forged edge from a squatted giver name shrinks another household's exploration floor** (§5.1, §7.1); the refusal above applies to a `key:` name whoever registers it, so those may take the shape and nothing here requires them to. **A name that is a key cannot be rotated**, so a member who replaces their key is a new household on this rail, and clause 53's recovery is about the ledger's envelope key rather than about this one. And **a household's protections are readable by anyone who holds its identifier**, which every merchant does. This section recommended an unguessable mandate label as the defence, and a refutation pass on 2026-09-16 showed the recommendation was already worth nothing: `GET /households/{id}/export` lists every mandate a household has, co-signers included, to whoever asks, and §16.2's `?household=` answers whether it has any. **A high-entropy label is still the better choice** and it is no longer offered as a defence; what closes the read is question 41's authenticated one.

**The defence before this was unguessability, and what the protocol publishes defeated it.** The reference hub derived both names from the credential id. An offer carries `household` and `mandate`, and `valence-merchant/1` (§14.1) hands every offer to the merchant that made it, so the two names reach every merchant the household buys from. A merchant that did not want to lose a household could register those names at the next host before the household arrived, and clause 52's promise was then broken by a stranger. Measured on 2026-09-16, on the engine: a squatted household name records that household's mandate with its ceilings and its co-signers, a squatted mandate identifier decides that household's set, and the household is refused its own key with `409 identity_exists` with no route that removes one.
...
A household moves its node by exporting it from one host and importing it at another (clauses 43, 52). The export carries the household's offers, settlements, notes, receipts, the lineage edges it is an endpoint of, and the register of what has confirmed each offer.

**It also carries what the route found in each physical box**, as `collections`, one row per offer that has one: what came back, what was used, what was missing with its notes, and when. The format's version is `valence-node/8` since 2026-09-20, when `decided_protections` was named below (§16.5, the third refutation pass over question 68); it was `valence-node/7` from 2026-09-19, when `payments` and `gifts_in_flight` were named below (question 61); it was `valence-node/6` from 2026-09-15, when `confirmations` was named below (question 50), and `valence-node/5` from 2026-09-14, when `missing` and `missing_notes` joined the row (question 46), and was `valence-node/4` from 2026-09-12, because a host that does not know a field would drop it and the member would arrive apparently intact. **The field exists because a refutation pass asked what a move does to §6.5's block**, and the answer was that it lifted: the offers moved with their `consumed` valences, the rows saying a collection had happened did not, and the receiving host presented the next box freely while the sending host held a block over a household that had left. **The merchant's export carried those rows all along** (§14.1), so the shop kept what the person lost, which is the direction clause 43 exists against. **It also carries what each decided set was decided under**, as `decided_protections`, one entry per offer that has a record: the window, the daily ceiling where the set may owe something, and which of those came from an earlier reading (§16.5). §16.5 makes recording them a MUST at the decision and reading the recorded ones a MUST at settlement and at withdrawal, and nothing carried them, so **a household lost both by exercising clause 43**. At the receiving host its mandates arrive as claims (question 56), so the live reads answer null, and with no record either the set had neither a window nor a fixed ceiling. That is the shape of question 50's `confirmations`, a register that lived only in what a host remembered and was dropped on the way. **No money moves on it today**: question 57 leaves a set whose money has not finished at the host that holds its reserve, so a decided set that arrives has settled, and what is carried is the household's own record of what it was held to rather than a live protection. An import MUST refuse `422 unscoped_protections` a record naming an offer the body does not carry, for question 52's reason, and MUST NOT replace one the host holds.

**An import MUST NOT replace a row the receiving host already holds**, for §14.2's reason: an import adds what the host does not have, and a host that overwrote its own record of a collection would let whoever composed the export decide what a box came back with.

**It also carries what has already confirmed each offer**, as `confirmations`, a map from an offer's id to the opaque tokens §10.5 records for it. Question 50, decided 2026-09-15. §10.5's rule that a confirmation is used once lives in what a host remembers, so a move that dropped the register would let a set taken back on the receiving host be decided again with the confirmation captured on the sending one. **A `valence-node/6` or later export MUST carry the field, and an import MUST read an offer the field does not name, in any version, as one with no confirmation recorded**, which §16.5 then refuses to withdraw. The reference engine carried the field from 2026-09-11, before `/4`, without this section naming it, so a host built from the text would drop it; that is the failure the version rule below names, and it is why the version moved. **What this does not close**: a set withdrawn on the sending host and moved without the register can still be decided again on the receiving host with the old confirmation, and the reference authenticates nobody at the import (§13.2), so whoever composes the body can leave the register out. The register narrows the replay to a move that drops it; it does not end it.
...
**It also carries what the household paid as a giver**, as `payments`, and the gifts it pays for whose money has not finished moving on the host it is leaving, as `gifts_in_flight` (question 61, decided 2026-09-19). A settlement lives with the recipient's offer (§12), so until this question a giver that moved took no record of what it paid, and its move named nothing left behind while its reserve was held at the old host: measured by the fifth refutation pass over question 57, the giver's move answered `left_behind: []`, the gift then settled at the old host charging the giver, and the giver's export at the new host had no settlement before or after. **A payment carries the offer, the presenter, the moment, the amount charged and the receipt, and MUST NOT carry the lines**, which name what the recipient chose and which clause 24 keeps from the giver. **A host lists as a giver's payments only the settlements it made itself**, beside the payments the giver brought: a recipient's import carries its settled gifts with their settlements, and listing those put a payment of any amount into the record of a giver that had posted nothing, measured by the third refutation pass over question 64 at 999,999. An import MUST add the payments it does not hold and MUST name every id in `gifts_in_flight` in its `left_behind` (§14.2); nothing is written for them. **What this does not do** is authenticate the rows: an import authenticates nobody (§13.2), so a payment row is a record the household brought and not proof of a charge, and it counts towards no ceiling (question 63).
...
- **Every key the import carries names itself.** An entry whose name is not the hash of the key it arrives with is refused (`422 name_is_not_the_key`) and the whole import with it. A carried key is used to verify the edges before anything is written and registered with them afterwards; a key this host already holds is left as it is.
...
- **Every confirmation names an offer the import carries.** Question 50, decided 2026-09-15. A register entry for an offer the body does not carry would add a token to one this host already holds, which unlocks the withdrawal §16.5 refuses and lets a captured signature decide the set again. Measured by a refutation pass the same day, which put a token on a held offer with a body carrying nothing else. A register that is not a map from offer ids to lists of tokens is refused with `400`, because an entry quietly skipped leaves its offer standing as a set moved without one.
- **Every other row belongs to the household on the path or to an offer the import carries.** Question 52, decided 2026-09-15. A settlement, a collection and a delivery name a carried offer, a note names a carried candidate, and a mandate and a recovery record name the household. Only offers and edges were bound before, and a refutation pass measured an import posted under one household replacing another household's mandate, raising its ceiling, emptying its co-signers and removing its cooling window, which is a way around clause 47 that needs no signature.
- **An edge or a mandate this host already holds is never replaced, and the same row arriving again is not a refusal.** An edge's signed bytes carry no id, so without this an import under one household could sign an edge of its own under another household's edge's id and erase it; the same edge arrives twice whenever both of its households move. A mandate is not replaced because loosening one needs its co-signers (§16.1) and an import carries no signatures; the same mandate arrives again when a hub recorded it before the node moved. **A different edge under a held id is written under an id the receiving host derives from the edge**, rather than refused, because refusing let one household squat an id and block another household's move. **A mandate this host holds is left as it is, and the arriving row is refused nothing**: a move carries no signatures, and keeping the tighter of the two froze a household out, because a ceiling of nothing, a lapse in the past and a co-signer whose key nobody holds are all tightenings and undoing them is a loosening that needs that key (§16.1). Refusing instead would let a mandate that arrived first block the household's move. The cost is that a tightening made on another host does not travel, and the household records it here with the signatures §16.1 asks for. A body naming one mandate twice is refused, because the second row decided what the first one did. **A mandate does not change hands**: an id this host holds for another household is refused with `409`, as an offer it holds is, and the numbers a mandate carries are whole and in range, as they are when one is recorded here (§16.1). **What that refusal cost** was that a mandate id was the client's to choose and there is no route that removes one, so a row filed first under a household that does not own it blocked that household's move to this host. Question 55, decided 2026-09-16: a mandate identifier now begins with its household's, which is that household's key, so only that household can compose one and the refusal has nothing left to catch. **Question 56, decided 2026-09-18: a mandate that arrives by a move is a claim, and does nothing until the household signs it at the host it arrived at.** This route authenticates nobody, so a mandate row can be planted under a household's own identifier; measured the day it was decided, a stranger's import placed a second mandate under a household's own identifier with a ceiling of 9,999,999 and no cooling window, beside that household's real one at a ceiling of 0 and a day's cooling, and a presenter naming the planted label got the loose one. **An implementation MUST write an arriving mandate as a claim**, and **a claim MUST have no effect**: it is not a mandate the host holds, `GET /_node/mandates/{id}` does not answer for it, it does not make §16.2's `has` true, and no ceiling, cooling window or lapse of its applies to anything.

**A household that has moved and not yet signed has set no protection here**, which is what §16.2 already says of every household before its first mandate, and the protections apply again when the person signs. **The first build of this decision said the opposite and it was wrong.** A claim counted among the mandates a household held, so that a move would fail closed, and a refutation pass measured what that bought: **one unsigned POST froze any household on the host**, one that held no mandate and had never moved included, because every offer naming any label was then refused. It stranded settlements already decided, the goods kept and the merchant unpaid, and nothing removed a claim. **A protection that anybody can switch on for anybody is not a protection**, and failing closed is the right direction only where the thing that closes it is the person's.

**A claim is an offer to sign and never a constraint.** An implementation MUST record a claim as a mandate when what is submitted is the claim itself, byte for byte, at the version the claim carries, which is what lets a mandate that was at version 3 where it came from become a mandate here. Anything else submitted under that identifier is an ordinary record, judged against what this host holds and not against the claim, and it clears the claim. **Neither half is optional.** Taking the claim's version without its terms lets a household record its own terms at that version and drop the co-signers it named where the mandate came from, which is clause 47 escaped by relocation; binding the claim's terms lets a planted row naming a co-signer nobody holds freeze that identifier for ever, which is the defect question 56's first attempt was refused for. **Clause 47's asymmetry is per host**: a signature is not retained (§16.1), so a host can hold a person to the co-signers they named only where they named them.

**An implementation MUST NOT keep a claim that has already lapsed**, because `record` refuses a lapsed mandate and the row could then never become one. **The export carries claims beside the signed rows and does not mark them apart**: an offer that moved with its mandate needs that mandate in the next export or the archive is invalid, and nothing downstream could verify the difference in any case, since every mandate row arrives at the next host as a claim whichever it was at the last.

**Question 57, decided 2026-09-18 and rebuilt 2026-09-19: money moves at the host that holds its reserve.** An implementation MUST leave behind an offer whose money has not finished moving, which is one at `drafted`, `presented` or `decided`, an `expired` offer with a line that still owes a settlement, or an `expired` physical box whose collection has not happened, together with every row of the body that names it, and MUST answer with the identifiers it left behind; it MUST NOT refuse the move for them. It MUST refuse a candidate that arrives with no verdict. Measured on the reference, an offer arriving at `presented` was taken with a `201` and **no reservation on the receiving ledger**; §6.4 makes the reserve the upper bound of what an offer may settle at, so such an offer's upper bound was one that ledger had never seen, and what stopped it charging was the ledger refusing `no_reservation`, which another adapter may not.

**The first build refused the offer, and a third refutation pass measured that it refused the whole move**: one presented offer stopped it, so a household with a weekly box could not move at all, and the reason given reached every decided offer not yet settled. The worst case was a box awaiting its statement, which moved, had its statement refused `no_reservation` at the new host, and left that presenter's next box refused there for good. **The cost of the rebuild is that §6.5's block is per host**: a presenter can deliver at the new host while an unsigned statement waits at the old one. This reverses the decision of 2026-09-12 that the block travels with the box, which was right that a move had lifted it and wrong about where a statement can be signed. A candidate arriving with no verdict is refused because `decide` reads a valence that is not `offered` as one already decided. **What this does not yet do is carry what was left behind once it settles**: a second move from the same host is refused `409` on the offers the first move already carried, so a household that moves with a box in flight never gets that box's record at the host it moved to. That is question 59.

**An offer that travelled settles only at the host it was presented at, and that includes a settlement of nothing.** An implementation MUST refuse `409 no_reservation` to settle an offer its ledger holds no reserve for. A fifth refutation pass settled an expired offer with every line returned at 0 on both hosts after a move, and one offer then had two settlements with two receipts. **Three costs of settling where the reserve is were measured by the same pass and are not closed here.** The daily ceiling of §16.3 counts per host, so a household that moves in the middle of a day can settle past it across the two hosts, and a ceiling tightened at the new host does not reach a set left behind at the old one. §5's floor is computed at the new host without the offers left behind, so a product already shown at the old host counts as never offered. The third, that a set decided with every line returned stayed behind until its presenter settled it at 0, was closed by question 62 (§6.4) for every set but a physical box nobody has collected, which still waits for its collection. **The first two were accepted as costs** (question 63, decided 2026-09-19): counting the settlements a move carries towards the new host's day would let anybody who can post an import fill a household's ceiling with settlements that never happened, because the import authenticates nobody, and a household that wants the old host's sets held to a tighter ceiling tightens its mandate there as well.

**An implementation MUST NOT offer a claim that has lapsed, and MUST let a live one take its place.** `record` refuses a lapsed mandate, so a claim that dies where it stands is one nobody can act on and one that holds an identifier against the household. The first build refused only a row that arrived dead and left the row that died later, which is every claim eventually: measured on the reference member service, whose acceptance writes a claim with a week's fuse, a household that did not sign within the week could never sign and no route removed the row. **No attacker was needed for that one, only a week.**

**A mandate with no signed history here starts at version 1 unless what is submitted is the claim itself.** For a day this section let the household state the version wherever a claim was held, and a third refutation pass measured what that reopened: a raw signature is not bound to a host, so any looser version the household had ever signed could be recorded at a host it moved to, over its own tighter claim. **This rule does not close replay and is not written to.** A fourth refutation pass measured three ways through it, none of them new to question 56: a claim planted with the terms of a looser version the household once signed lets that signature record as the claim itself; restarting at version 1 lets the versions the household signed elsewhere follow in order, since a version number is per host; and the reference hub writes version 1 after every move, because it reads the claim's 404 as nothing. A co-signer does not stop any of them, because whoever relays a submission holds the co-signer's signature too. **What closes replay is binding the signature to a host, which is question 58**; until then a household's signed history can be replayed at any host that holds none of it. **An implementation MUST NOT keep a claim at a version no household reaches**, because signing it would leave no room to follow and freeze the mandate, and **a version MUST have room to be followed**: a version at or above 2^53 does not increase when one is added to it, so the version stops rising and an old signature replays onto a new record, which is the one thing §16.1 puts the version in the bytes to prevent.

**A household whose only key is an authenticator's own credential cannot sign at another host, and its mandates therefore do not move.** An assertion names the relying party it was made for (§10.5), `record` checks that against the host doing the verifying, and §13.2 makes the identifier the name of the key, so enrolling a new credential at the destination produces a different household rather than a way to sign. **A household that intends to move holds a key it can sign with anywhere**, which is what the reference hub does by carrying that key inside the passkey's user handle rather than using the passkey itself. Named by a refutation pass on 2026-09-18, which measured it as the shape the reference member service runs: such a household arrives with its out-of-network ceiling, its daily ceiling and its cooling window off together, and no act of its own turns them on. **This is a property of that deployment's key and not of this route**, and it is written here because it is invisible from either end.

**Carrying the signatures instead was refused twice over**: §16.1 requires that a signature is verified at submission and not retained, and an assertion names the host it was made for, so a member who holds a passkey and nothing else, which is every member who joined through a hub, could move no mandate at all.

**The same edge is the same edge wherever this host filed it**, so a copy re-keyed on one host and the original on another are one edge and are not both written, which is what keeps a squatted id from forking a gift's lineage; two separate gifts that sign the same bytes are not one edge, so only the id an edge arrives under and the ids derived from it are looked at. **An imported edge's `attested` is this host's to decide**, from whether the giver's key is root-endorsed here, as it is on arrival (§7.1), and its fields are strings with one of the four kinds, because a kind of `["gift"]` encodes to the bytes a gift does and verifies. **A body naming one row twice is refused** (`400`): a revoked permission beside a live copy of itself reads as revoked and goes on granting. **A grant that arrives by a move meets what a grant made here meets** (clause 9, clause 38), its scope included: a grant whose scope names nothing is refused, because the reference then raised on every later read of that household's permissions and no revocation could remove the row. Which mandate an offer reads is bound to its household by §16 (question 54). **What that does not bind** is which of a household's mandates an offer names: a household may record a second mandate under a new id with its own signature alone, and a presenter that names the looser one leaves the co-signers of the other out. **A move no longer brings one unsigned**, which was the half of question 56 that closed on 2026-09-18. **Receipts, the recovery log, permissions and queries are added to**, by `ref` or `id`, and never replaced, so a second import cannot erase what the host recorded.
- **An offer this host already holds is never changed.** An import adds what the host does not have, and a move lands on a host that has none of it, so nothing about a move is touched by this. What it refuses is the other thing the route can do. **Measured 2026-09-11**: a node handed to a host said an offer was `decided` and every candidate `kept`, with no signature anywhere in it; an offer sitting at `presented` became that, and settling it charged for goods nobody had agreed to. Clause 35 makes a confirmation the person's signature and §10.5 refuses a decided set without one, and this route walked past both. It was the settled case alone before, which §2.1 already required. **An offer this host holds exactly as it arrives has been carried already, and is not a refusal.** Question 59, decided 2026-09-19. A move leaves behind what has not finished moving money (question 57), so once that settles the natural next step is to move again from the same host, and that body carries every offer the first move brought; refusing them refused the move, and the record of the box left behind never reached the host the household lives at. **An offer that differs in anything is still refused with `409`.** **So is every row the body carries for an offer already carried** (a settlement, a collection, a note, a confirmation token) **unless this host holds that row as it arrives**, because an import authenticates nobody and repeating an offer must not be a way to put a settlement on a withdrawn set, a note in the household's hand or a token beside a held one. A delivery is the exception: its status moves as the parcel does, so only its carriage and code are compared, and the host keeps its own row. Nothing of an offer already carried is written again. The cost is that a row the sending host changed after the first move, a note added there to a carried candidate for one, refuses the second move until it is put right.

**Refusing the whole import means that a refused import writes nothing**, so an implementation decides every refusal above for the whole body before it writes a row, and a move refused for one row can be retried once that row is put right. A row that is not the shape an export gives it, or whose key is not a string, is refused with `400` in the same pass, because a store that keeps a disk fails such a row only after it has been written to memory; a key that is not well-formed Unicode is refused for the same reason, since it binds and does not read back as itself. **An accepted import stands whole or not at all.** Question 53, decided 2026-09-16. What can still fail after every refusal is decided is the store: a locked database, a full disk, a process stopped between two rows. An implementation MUST write an accepted import so that such a failure leaves none of it, in what it serves and in what it keeps, and so that the same move can then be retried; the reference writes it inside one database transaction and puts back in memory what the transaction rolled back. Question 51, decided 2026-09-15: the reference wrote row by row until then and stopped at the first refusal, so a move refused at an edge kept the offers written before it, lost the collections and mandates after it, and could not be retried because those offers were now held. An edge whose giver's key the receiving host has not attested was enough, so a move could fail that way with nobody at fault. An implementation MUST refuse the whole import with `422` when any of these fails, `409` for an offer it already holds that differs from the one arriving, and MUST refuse an export whose `format` it does not recognise with `400`. An implementation MAY read `valence-node/4`, whose collections carry no `missing` and no `missing_notes`, as a collection that recorded nothing missing, and MAY read `valence-node/5`; either may carry no `confirmations`, and §14 says how an offer it does not name is read. Added on 2026-09-09: the reference engine's import verified none of it, and an adversarial pass planted a forged edge that made a product unofferable to a household that had never seen it.
...
**The engine's route is `GET` and `POST /households/{id}/leave`**: the first answers `{household, blockers}`, the second deletes or refuses `409 leave_blocked` with them. **The `POST` carries the household's own signature or assertion over `["valence.leave.1", household, <relying party>, at]`**, with `at` the moment it asked, verified against its registered key, and answers `422 unsigned`, `422 stale_request` (more than five minutes from the host's clock) or `422 bad_signature` otherwise. **The moment is what stops a signature once seen from deleting the household again** after the same key enrols once more. The routes that only read authenticate nobody, as the export beside them does; a deletion cannot, because a hub that carries the engine's routes would otherwise delete any household anyone named. A hub that holds the member's signature itself, as the reference member API does, calls the implementation rather than this route.
...
- a pending mandate change or recovery request it started (a permission request is a merchant's to start, and is deleted with the rest rather than counted: counting it let any merchant keep a household from ever leaving, measured by a refutation pass on 2026-09-22);
...
**What is deleted.** Everything this host holds that is the household's alone: its principals, credentials, passkeys, sessions, mandates and claims, the permission ledger and its queries, its private records, its recovery configuration, and its operations and reviews. And the host's copies of what it shares with merchants: its offers, settlements, corrections and the records of what became of its refunds (§6.6a), receipts, notes, collections, deliveries and carriage quotes. A merchant keeps its own copy through `valence-merchant/1` (§14.1), and the duty to keep a record of a sale is the seller's, not the host's.

**What another household holds is not the leaving household's to delete.** A gift the household gave or received stays in the other household's records unchanged, with its lineage edge, because the other household's receipt and its exploration floor (§5.1) rest on them, and changing the signed bytes would break the signature §14.2 verifies when that household moves. A hub MUST display the departed party as a member who has left, not by any identifier. **The departed household's public key is retained exactly as long as a remaining household's edge or gift needs it to verify**, and removed with the last such row; a public key without the credentials, sessions and mandates deleted beside it identifies no one to this host. **A shared row is kept for the party still here, so when that party leaves too, its departure deletes the row** and, with it, the last reason to keep the first party's key; a row both parties have left is nobody's record.

**Afterwards.** A presenter route naming a deleted offer answers `404 not_found`, the same as an offer that never existed, so a merchant learns nothing about a household from asking; a correction for one (§6.6), or a record that its refund came back (§6.6a), is refused the same way and is the merchant's to handle outside the protocol. An implementation MUST keep, for its own audit, the fact that a household identifier was deleted and when, and nothing else about it, for seven years: the period a seller keeps the records of a sale (法人税法施行規則 59条), so that a question about one of the household's purchases can still be answered with when the household left. The same passkey enrolling again later creates a household again with nothing of the old one's history, because none remains.

**What the first refutation pass changed, on 2026-09-22.** Each of these was measured on the running code.

- **The same key coming back is the same household.** An identifier is the name of a key, so a passkey that enrols again after leaving returns under its old identifier, and an implementation MUST record the return beside the departure rather than leave the household counted as gone. A departure row read as gone swept a returned household's key when someone else left. A returned household sees the gifts kept for the other party, because they name it; that is the cost of the rule above and is accepted.
- **A note is the writer's (§7.6b) even on a row kept for another household**, so the leaving household's lines go from it.
- **A presenter MUST NOT write an offer to a household the host holds no member for**, and answers `404 household_unavailable` alike for one that never enrolled and one that left. A merchant knows a departed household's identifier from its own export, and an offer written to it would return as history if the key enrolled again.
- **A mandate MUST NOT name a departed household as a co-signer** (`422 co_signer_departed`): it can sign nothing, so the mandate could never be loosened.
- **A deletion refused after the signature was verified spends the review.** Refusing by rolling everything back left the review in place, so the same assertion deleted the account later, once the blocker cleared.
- **The audit row goes after seven years once no remaining row names the household**, and a row still named stays, because it is what tells a later departure that the other party has left. A deletion looks only at the parties of the rows it deleted, not at every departure the host has had.

**Closed on 2026-09-23**: a remaining household's move verified a departed giver's edge at a receiving host that did not hold the key. The export now carries the keys its edges need (§14, `valence-node/11`), which is also why the departed household's key is retained here while a remaining row names it.
...
```
mandate
  id                       the household's identifier, a full stop, and a label (§13.2)
  household                `key:` and the base64url SHA-256 of the household's public key (§13.2)
...
Every version is signed by the household over the canonical form: `valence.mandate.2` and the host the version is recorded at, which is the relying party that host asserts for, then the fields above, one per line, in the order listed, with `co_signers` sorted, **each item percent-encoded**, and joined by commas, an absent `ceiling_daily` or `cooling_seconds` as an empty line, and the version inside the bytes so that an old signature cannot be replayed onto a new record. This sentence named no escaping until 2026-09-11, and the escaping is why the next paragraph but three exists.

**A co-signer is named by the key it signs with** (§13.2, question 55): the names in `co_signers` are `key:` names, because whoever holds the name holds the signature this section asks for.
...
**Bringing `lapses_at` forward loosens a mandate whose previous version names co-signers** (decided 2026-09-19, after the first refutation pass over question 68). It takes their protection away sooner, so it needs every one of them, as raising a ceiling does; pushing the lapse later stays a loosening as before, and a mandate that names nobody is the household's alone in both directions. It counted as a tightening until then, and question 68 reads the tightest value only among mandates that have not lapsed, so the household undid question 68 with it: measured on the reference, a household holding `.1` with an out-of-network ceiling of 0, a daily ceiling of 0, a day's cooling window and a co-signer moved `.1`'s lapse to a second away, alone, and once `.1` had lapsed an offer of 1,200 entirely outside the network presented, was decided and settled at once under a loose `.2` it had recorded alone. **What it costs** is that a household cannot end a co-signed mandate early by itself, including one whose co-signer it has lost; how long that can hold it is bounded by the next paragraph.

**A version's `lapses_at` is at most 400 days after it is recorded** (clause 58, decided the same day, widened from 366 on 2026-09-20). An implementation MUST refuse with `422 lapse_too_far` a version whose `lapses_at` is further than 400 days past the moment it records it, and SHOULD name its own clock in the refusal, because the party whose date is wrong is usually the device the lapse was computed on. **The bound was 366 days for a day**, a year plus the minutes between two clocks, and the second refutation pass over question 68 measured what that day of slack is spent on: the reference hub computes the lapse on the member's own device (`Date.now() + 365` days), so a phone two days fast had every button on its protections screen refused, tightenings included, because each writes a whole version. The bound exists to bound the freeze the paragraph below describes, and 34 days more of a freeze that already runs a year is a smaller cost than a member who can set no protection at all; a bound tight enough to be a tolerance budget is measuring the wrong thing. Question 68 made every mandate a household holds bind every offer it makes, and the paragraph above made its lapse the co-signers' to bring forward, so a mandate naming a co-signer nobody holds (a typo, or a key the person lost) holds its household until it lapses, and nothing bounded that: the same refutation pass recorded a mandate lapsing in the year 9999, whose daily ceiling of 0 then refused everything under every label. **A household that has lost a co-signer is therefore held for at most 400 days**, and that is the cost: nothing in this section lets it out sooner, since refusing a co-signer whose key is not registered at record time would not reach one lost afterwards. It is also the cost to every household of renewing at least yearly, which clause 58 already asks of a standing mandate. A claim that arrives by a move with a lapse beyond the bound can never be signed here, and the household records its own version 1 instead (§14.2).

**A protection is a whole number and never below zero, and a value outside the bounds of this section is read as the nearest one inside them** (decided 2026-09-20, after the third refutation pass over question 68). An implementation MUST refuse with `422 not_a_protection` a version whose `ceiling_out_of_network`, `ceiling_daily` or `cooling_seconds` is not a whole number or is below zero, and MUST apply that refusal wherever a version is recorded and not only at the HTTP route. **The bounds were in one place and the values were used in three.** `cooling_seconds` was bounded below at the route alone, and the pass recorded -1, 0.5 and a ceiling of -5 through the register, which is what the acceptance flow of a deployment and every test call; a negative window is not absent, so §16.5's longest selects it wherever it is a household's only one, and the screen shows a window no engine can honour. The bound above a window was enforced at the same one place, so a hub that is not this reference, and a row recorded before that bound existed, still fixed a decided set for thirty years. **At the reading end the value is read and not refused**: an engine MUST read a `cooling_seconds` above the bound of §16.5 as that bound, a negative window or ceiling as zero, and a fraction toward the protective side, up for a window and down for a ceiling. Refusing was considered and rejected for the reason §16.5's paragraph on the statement gives: a settlement that can never be made blocks that presenter's next box under §6.5, so a household would lose its deliveries over a protection it set before this section narrowed what one may hold.

**A mandate that arrived by a move is signed here before it is one** (§14.2, question 56, decided 2026-09-18). What a move carries is a claim, and this section is the route by which a claim becomes a mandate: the household signs the claim's own canonical bytes, at the version the claim carries. **Both shapes are bound to the host**, the assertion by the relying party inside the authenticator's data and the signature by the host line of the canonical form, so a claim is signed again at the host it arrived at and a signature made there records nowhere else. **Question 58**, decided 2026-09-19. The form named no host until then, so a signature made for one host recorded the same claim at another, and whoever relayed a submission once could do that without the person being present; a fourth refutation pass measured the reach as every version a household had ever signed, at any host that held none of its history, through three routes a version rule could not close (a claim planted with an older version's terms, a restart at version 1, and the reference hub writing version 1 after every move). The cost is every signature over the form, and that two hosts that assert for one relying party share one name here; a deployment that runs two hosts under one relying party MUST treat them as one host for this purpose. **The decided set (§10.5) and the settlement statement (§6.5) name no host, and what holds each to one is that the host mints the offer id**: an offer reaches another host only by a move, and a move leaves behind every offer a signature could still act on (§14.2, question 57). An implementation MUST mint offer ids itself and MUST NOT take one from a presenter, or it inherits this question for both forms. A lineage edge (§7.1) is bound to no host on purpose, since it states a public fact and is verified again wherever it arrives. Measured by the first refutation pass over this question.
...
**The reference hub sends the first shape and not the second, since 2026-09-16.** Question 55 made a household the name of a key, and a WebAuthn `get` returns the credential id and never the public key, so the key a member carries between devices is one the hub makes and keeps inside the passkey's user handle. What the engine loses by it is the one thing the assertion shape gave it: a signature it could tell had followed a person's verification, because the challenge was the bytes and the authenticator signed its own data. **An implementation still MUST accept both**, and the shape is exercised by the conformance suites and by the iOS client rather than by the reference member. Named by a refutation pass the same day.
...
**Two things about the cooling window are true and neither is obvious** (§16.5). It is read when a set is taken back or settled and not when it was signed, so a person who sets a window afterwards may take back a set that was final when they signed it, and a presenter entitled to settle at once is deferred for the length of a window the person added later. That direction is the person's to gain, and the other is not: shortening needs the co-signers. **Taking a set back asked for no signature at all** until 2026-09-20, so whoever held the offer id could do it, the presenter included, and a presenter refused a withdrawal under a decided offer could reach `presented` this way and withdraw from there. It was measured on 2026-09-11, named here rather than fixed, and the third refutation pass over question 68 measured what question 68 then did to it: the window is the interval in which the route is open, and question 68 made the window the longest across every mandate the household holds. §16.5 now asks for a signature over taking the set back. **That is not §15's nonce**, which binds a request to a person for every route at once and is still open; it binds this one request to the household's key and to the set that stands.

### 16.2 The ceiling

At presentation, an implementation that holds a mandate for the offer MUST refuse with `422` when what the offer could cost at merchants the registry does not list exceeds `ceiling_out_of_network`, and MUST refuse when the mandate has lapsed.

**The ceiling is the tightest across every mandate the household holds, and not the one the offer names** (question 68, decided 2026-09-19). An implementation MUST read, for an offer with no giver, the tightest `ceiling_out_of_network` among the mandates it holds for the offer's household that have not lapsed. Clause 47 is what this is worth. A version that loosens needs every co-signer the previous version named, and a register asks for them only where the identifier already has a row, so a household holding `<household>.1` with a ceiling of 0 and a co-signer recorded `<household>.2` at version 1, alone, with any ceiling it liked, and presented under the second: measured 2026-09-19, loosening the first was refused `unsigned` and an offer of 1,200 entirely outside the network presented under the second. **The signing rules for recording are unchanged**: a household records a new label alone, which is the reason question 56's first attempt was refused; what changed is that a mandate nobody else signed governs nothing by itself. **This sentence said until the same day that a household is never frozen out by a co-signer whose key nobody holds, and the first refutation pass over question 68 refuted it**: such a mandate now binds every label, and only its co-signers can bring its lapse forward (§16.1), so it holds the household until it lapses, which §16.1 bounds at 400 days. **What it costs** is that a household cannot hold one loose mandate beside one tight one and use each in its place: the tight one binds everything, and a household that wants a loose label has to loosen the tight one, which is a loosening and needs its co-signers. A hub answering `GET /_node/mandates?household={id}` MUST carry `ceiling_out_of_network`, the tightest, beside `has`, and an engine MUST refuse where the field is absent rather than read silence as no ceiling, for §16.3's reason. **The lapse and the section below are still about the mandate the offer names**: an offer whose named mandate has lapsed is refused `mandate_lapsed`, and one naming a mandate the household does not hold is refused `mandate_unknown`, whatever the household's other mandates say. The registry is what "in the network" means (§17); a person's limit on the rest is applied inside their own mandate, which is clause 46's ceiling under clause 47's structure. **This line cited clause 55 until 2026-09-11**, and clause 55's exclusion is of a fork of the hub's software, not of a merchant the registry does not list.

**The out-of-network ceiling is the payer's** (question 65, decided 2026-09-19), as the daily ceiling is (question 60, §16.3). For a ceremonial offer (§12) an implementation MUST compare what the offer could cost outside the network with the tightest `ceiling_out_of_network` among the mandates it holds for the giver that have not lapsed, and MUST NOT refuse a gift on the recipient's; the recipient's own lapse still refuses, since it is the recipient's mandate the offer names. A giver that holds no mandate **here** sets no ceiling, as an unknown mandate does, **and that is where this protection stops**: a giver whose household lives at another host is indistinguishable from one that set nothing, because §16.2's route answers for the mandates this host holds and nothing reads another host's (question 41's authenticated read is what would). Measured by the fifth refutation pass over question 66: a gift of 240,000 entirely outside the network presented for a giver at another host. The ceiling it replaces was the recipient's, which is the wrong party's, so this is narrower than it looks rather than a loss; it is written here because an implementation MUST NOT be read as bounding a gift whose giver it holds nothing for. `GET /_node/mandates?household=` answers the tightest as `ceiling_out_of_network` beside `ceiling_daily`, and an engine that reads it from a hub MUST refuse a hub that answers without it rather than read silence as no ceiling. The third refutation pass over question 64 measured the mirror of question 60 one screen above the gift check: a recipient with a ceiling of 0 had a gift it pays nothing for refused, and a giver with a ceiling of 0 presented a gift charging it 1,200 outside the network.

**An implementation that holds any mandate for the offer's household MUST refuse with `422 mandate_unknown` an offer naming one it does not hold.** Question 56's first half, decided 2026-09-16. An offer whose mandate this implementation does not hold is otherwise left alone: a deployment may carry mandates elsewhere, and refusing every offer whose mandate is unknown would be a gate rather than a protection, and a household that has set no protection here is every household before its first mandate. **What the unqualified rule cost was measured the day it was qualified**: a presenter naming any label after the household's own prefix got an offer with no out-of-network ceiling, no daily ceiling and no cooling window, having recorded nothing, imported nothing and forged nothing, and the household could not see it, because the decided set's signed bytes name the offer and its candidates and not the mandate (§10.5). An implementation that does not hold the household's mandates itself asks the hub **whether it has any**, over `GET /_node/mandates?household={id}`, which answers `has` and not the rows, and a hub it could not reach is not a hub that says there are none, for §13.2's reason. **One bit and not the rows**, because that route is reachable by whoever holds a household identifier, which every merchant does. **It is not a defence of a household's protections and this section does not claim to be one**: `GET /households/{id}/export` already hands every mandate a household has, co-signers' keys included, to whoever asks, and a presenter can read the bit anyway by presenting under a label of its own and seeing which answer comes back. What closes that read is question 41's authenticated one. The bit is narrow because a route should carry what its caller needs and no more, which here is one caller and one bit. **A hub that does not have the route refuses every such presentation**, because a 404 there is not an answer, and on a split deployment that is every household before its first mandate. Fail-closed and written down rather than measured later. **The refusal belongs wherever the mandate is read and not at presentation alone.** It was at presentation alone for an hour, and an offer presented while the household had set no protection still settled without the one it set afterwards, which is the ordinary order for a new member: the presenter had only to present first. §16.5 already reads a cooling window when a set is taken back or settled and not when it was signed. **What that costs** is that such an offer can no longer settle at all, and the presenter has to make another naming a mandate the household has; nothing settles under protections that were not applied, which is the direction to fail in.
...
**The ceiling is the tightest across the payer's mandates** (question 60, decided 2026-09-19, and question 68 the same day). For an offer with no giver the payer is the household, and an implementation MUST read the tightest `ceiling_daily` among the mandates it holds for that household that have not lapsed, rather than the one the offer names: the escape §16.2 describes is the same here, and a household that had tightened one label settled freely under a second it recorded alone. For a ceremonial offer (§12) the payer is the giver, so an implementation MUST read the tightest `ceiling_daily` among the mandates it holds for the giver that have not lapsed, MUST sum the giver's day and MUST report the settlement to the giver's day, and MUST NOT refuse a gift on the recipient's ceiling. A gift names no mandate of the giver's, which is why the tightest governs. Measured by the fifth refutation pass over question 57: a giver with a ceiling of 500 was charged 1200, and a recipient with a ceiling of 500 had a gift refused that it pays nothing for. The recipient's mandate still governs what the recipient does, which is its cooling window and whether the offer names a mandate at all. A hub answering `GET /_node/mandates?household={id}` MUST carry `ceiling_daily`, the tightest, beside `has`, and an engine MUST refuse where the field is absent rather than read silence as no ceiling.

**A decided set keeps the daily ceiling it was decided under** (decided 2026-09-19, after the first refutation pass over question 68). When a set is decided, by the household's signature or by a collection that resolves a box's last line (§11.2), an implementation MUST record the tightest `ceiling_daily` among the payer's mandates live at that moment, and at settlement MUST refuse on the tighter of that value and the tightest live at settlement. A later lapse therefore does not remove it, whether the mandate reached its own date or its co-signers agreed to bring the date forward (§16.1). The paragraphs above read only mandates that have not lapsed, and the pass measured what that did to sets already decided, with nobody acting: a set decided under a daily ceiling of 500 was charged 900 once its one mandate had lapsed, and main had not done this, because it read the named mandate whether or not it had lapsed. **Recorded and not recomputed.** Recomputing against the decision's moment reads the rows as they stand at settlement, and a version recorded after the decision, which a loosening with its co-signers' signatures is, replaces the one the set was decided under; a recorded value is moved by nothing. A tightening after the decision still reaches the set, through the live half, as it always has. **What it costs**: a loosening the co-signers agree to does not reach a set already decided; and the engine asks for the ceiling when the set is decided as well as when it settles, so a decision waits on one more read. **A decision is never refused because that read failed, where this host has read that household before** (decided 2026-09-20, after the second refutation pass, and rebuilt the same day after the third). An implementation MUST record, for a read it could not make, the last value it read from the source for that household, MUST record which of the values it recorded came from an earlier reading rather than from the decision, and MUST NOT refuse the decision. **Where it has never read that household from the source it MUST refuse**, because there is nothing to fall back to and the alternative is a set decided under no protection at all. The first build fell back to no protection for every failed read, and the third pass measured what that is worth to the party holding the connection: on a split deployment (§13.1) the engine is the presenter's side and the decision arrives through it, so one silent read, at a moment that party chooses, took away a day-long cooling window and a ceiling of 500 and settled 900. **A protection that one party can switch off by not answering is not one.** The cost of the other end is measured too, and it is the case above run against a hub that has never carried `cooling_seconds`: the recipient cannot decline a gift at all, §12 defaults it and the giver is charged, so a deployment whose hub predates this sells nothing to that member until it is upgraded. The first build refused, and the pass measured the cost against a hub built one day earlier, which answers the household route without `cooling_seconds`: a household's refusal of a gift was refused `hub_refused`, the offer reached its expiry with every line still `offered`, §12 defaulted them, and the giver was charged 700 for goods the recipient had declined, where `origin/main` settles the same decision at 0. **A refusal must never become a purchase**, and a decision is the one place in this specification where the message is usually the person saying no; the settlement's own reads still refuse, so nothing settles under a protection that was not applied. The same holds for the collection route, which that hub refused outright. The ceiling is still not asked for at a decision that can owe nothing, a digital set with nothing kept that is not a gift, because a value that can never refuse is not worth a round trip; **that carve-out was written as a protection against a hub predating question 60 and was never one**, since the cooling read beside it was unconditional and rides on the same route. **What it does not reach**: a set that reaches its end by expiry and not by a decision (a ceremonial default, §12, or a box past its deadline, §11) has no decision to record at, and is held to the live ceiling alone, as before; and a store written before the record holds nothing recorded for the sets it already has.

**The ceiling counts per host** (question 63, decided 2026-09-19, accepted as a cost). A household that moves in the middle of a day can settle past its ceiling across the two hosts, because the settlements a move carries are not counted towards the new host's day, and a ceiling tightened at the new host does not reach a set left at the old one (§14.2). Counting the carried settlements was refused: the import authenticates nobody (§13.2), so anybody who can post one could fill a household's day with settlements that never happened. **That reason does not describe the reference, and this was found the same day.** `POST /households/{id}/settled`, the route an engine reports to the hub through, takes any row from any caller and replaces a row by its offer id, so the same anybody can already fill a household's day, and zero it: the third refutation pass over question 64 wrote a household's two settlements back at 0 and settled past its ceiling, and wrote 999,999,999 and had every settlement refused. **A ceiling is a protection only once the hub authenticates who reports to it**, and since 2026-09-22 it does: question 67 was decided that day without waiting on question 41, by authenticating the engine rather than the household (§13.2). A hub that accepts settled rows from any party other than its own engine has no ceiling. Question 63's reason still describes the import, which authenticates nobody.

**One payer's settlements run one at a time** (question 66, decided 2026-09-19). The ceiling is read, the ledger is told and the day is told, and wherever one of those waits on the network two settlements for one payer both read the day before either is counted: measured with a ledger whose commit is a round trip and with a day read over HTTP, 2,700 settled against a ceiling of 2,000. An implementation MUST serialise the read, the check, the commit and the report for each payer within one process. Two processes settling for one payer remain the non-atomic case the paragraph below names. **A settlement retried after its report to the day failed MUST be reported again**, and the day keeps one row an offer, so the retry cannot count it twice; the reference answered a retry with the settlement and told nobody, and the day stayed short by that amount for the life of the host. A settlement a move carried is never reported, for question 63's reason. **A retry while the hub cannot be reached answers `422 hub_unreachable` on every attempt, for a settlement that is written and whose ledger has moved**, which is the direction this section chose: a presenter reading the refusal retries, and the retry is safe because the day keeps one row an offer. **An implementation MUST record where each settlement was made, as it makes it**, and MUST report to the day, and list as a giver's payment, only the settlements it made itself. **The record is positive.** A settlement that carries no record of where it was made is one to leave alone, because the absence of a record is the only thing a store can be sure of: three refutation passes over question 66 refuted three ways of inferring the answer afterwards (a settlement with no reservation on the ledger, a ledger that says it keeps its rows, and a store's own emptiness at its first open), and the third of them measured what reading silence as "made here" costs: an unauthenticated `POST /offers/{id}/settle` on a store written before the record told the payer's day 999,999 on a settlement a stranger had imported, and the payer's own gift was then refused on its daily ceiling. **A store written before the record cannot say which of its settlements a move carried and MUST be rebuilt**; the reference writes what the store can say about itself when it opens it, once, and warns. **What a host that keeps serving such a store does to its givers**, since nothing can make it rebuild: every gift it settled before the record is no longer listed as its giver's payment, in the giver's export either, so a giver that moves afterwards carries only what was settled since; and a retry of one of those settlements tells no day. Measured by the sixth refutation pass. The records are not lost from the store, only from what the host will vouch for, and a store rebuilt from the households' own exports has them back. The rule for question 66 is proven by the engine's unit tests and not by a conformance probe: the reference's in-memory ledger resolves every wait at once, so two settlements over HTTP never overlap, and a failed report needs a hub that fails on cue.
...
A decided set under a mandate with `cooling_seconds` set does not settle when it is signed. `POST /offers/{id}/settle` MUST refuse with `422 mandate_cooling` until `cooling_seconds` have passed since the decision, and `DELETE /offers/{id}/decisions` withdraws the set before then, returning the offer to `presented`. Withdrawing is the person's alone and needs no co-signer. **The refusals of this section name themselves too**, as §16.6 requires of its own: `422 no_cooling` where the mandate sets no window, `422 cooling_over` once it has closed, and `409 not_withdrawable` where no confirmation is recorded for the set, whether a collection resolved it or it arrived by a move without one (question 50), and where a physical box is collected or past its expiry. **`409 not_withdrawable` is decided before the mandate is read**, so an offer it covers is refused with it whatever the mandate's window says. **They were unnamed here until 2026-09-13**, and a name the specification does not carry is one no probe has to assert and no second implementation has to produce: `cooling_over` lost its only assertion that day when the probe holding it was rewritten, and nothing could say so.

**The window is the tightest across every mandate the household holds, which is the longest** (question 68, decided 2026-09-19). An implementation MUST read the longest `cooling_seconds` among the mandates it holds for the offer's household that have not lapsed, at `POST /offers/{id}/settle` and at `DELETE /offers/{id}/decisions` alike, rather than the one the offer's mandate names; a mandate that sets no window contributes none and is not a window of zero. This is the one place in §16 where the tighter value is the larger one, and it is the same escape §16.2 describes: a household that had set a window on the label its offers named settled at once under a second label it recorded alone, and could not take that set back at all, because the second label's window was the one read. A hub answering `GET /_node/mandates?household={id}` MUST carry `cooling_seconds`, the longest, and an engine MUST refuse where it is absent rather than read silence as no window. **The window is asked for only where it can bar the settlement** (decided 2026-09-20, after the third refutation pass over question 68). It was asked for at every settlement, including one whose application is the household's signature over a statement and which no window bars, on the reasoning that a rule with nothing to disturb rests on nothing a break could catch. The pass measured the other end of that sentence: a hub predating this answers the household route without `cooling_seconds`, the read refuses, and on a statement settlement the refusal leaves the statement unsigned, so §6.5 blocks that presenter's next box and nothing times it out. **A protection this household may not even have set stops its deliveries**, which is a cost §6.5 was never meant to carry. The rule that a window never bars a statement is proven instead by a break of the test itself, which is the one place it has behaviour. **What it costs** is that a household cannot keep a short window for one kind of offer and a long one for another, and that a window it set once reaches every offer it makes until it is shortened, which is a loosening.

**A decided set keeps the window it was decided under** (decided 2026-09-19, with §16.3's paragraph of the same day). When a set is decided, an implementation MUST record the longest `cooling_seconds` among the household's mandates live at that moment, and at `POST /offers/{id}/settle` and `DELETE /offers/{id}/decisions` MUST read the longer of that window and the longest live then. So a lapse does not end a window already running: the first refutation pass over question 68 measured a set taken back 11 seconds into a day's window refused `no_cooling`, and a second set settled at once, once the one mandate that set the window had lapsed, whether by its own date or because the household brought the date forward alone (§16.1 now forbids the second). A window the household lengthens after the decision still reaches the set, as §16.1's paragraph on the cooling window says. The window is recorded rather than recomputed for §16.3's reason, and costs what §16.3's does: a shortening the co-signers agree to does not reach a set already decided, and a decision waits on the read. **A decision is never refused because that read failed**, where this host has read that household's window before; what could not be read is recorded as the last value read for it, and marked as having come from an earlier reading, for the reason §16.3 gives.

**A recorded `cooling_seconds` is at most 30 days** (decided 2026-09-20, after the second refutation pass over question 68). An implementation MUST refuse with `422 cooling_too_long` a version whose `cooling_seconds` is above 2,592,000. The paragraph above makes the window a set was decided under outlive the mandate that set it, and nothing bounded the window itself: the pass recorded thirty years, decided a set, dropped the window a second later and alone, and left that set unable to settle until 2056 while the household's own mandate showed no window at all. The presenter is given no cause it can see and its reserve expires under the hold. Lengthening a window is a tightening, so this needs no co-signer and no attacker; on `origin/main` the same sequence settled, because the window was read live and the decision recorded nothing. **What it costs** is the household that wanted a window longer than a month, and a window is time to change one's mind about a set just signed. A claim that arrives by a move with a longer window can never be signed here, as §16.1's bound on the lapse already says of a lapse beyond its own.
...
**A set that reached `decided` through a collection cannot be taken back.** Question 43, decided 2026-09-13. `DELETE /offers/{id}/decisions` MUST refuse, with `409`, an offer whose state came from a collection rather than from a signed set, which an implementation can tell because no confirmation was recorded for it. Withdrawing is the removal of a commitment, and **where the household signed nothing there is no commitment to remove**; this refusal is therefore decided before the signature the paragraph below asks for, so a set nobody signed answers `409` rather than being asked for a signature over a commitment that was never made. The route did three things instead. The box returned to `presented` with the collection's verdicts intact, so it reached no section of the household's own list and was invisible until it expired. The household's signature over the original statement was then refused as out of state. And §6.5's block lifted, because the block counts only boxes in `decided` and `expired`, so the presenter's next box presented at once; a presenter that then withdrew the box left the consumed goods charged to nobody. The route asked for no signature at all and a presenter knows its own offer ids, so this was reachable by the party it costs the household most. Found by a refutation pass on 2026-09-12.

**A set that arrives by a move with no confirmation recorded is refused the same way.** Question 50, decided 2026-09-15. §14 makes the register part of the export, and an offer it does not name is, to the receiving host, a set with no confirmation recorded; the question 43 test above reaches it without change. Refusing costs that household its cooling window on that set. **The alternative considered and refused** was to tell a collection by its row instead of by the register, which keeps the window and lets a set withdrawn on the receiving host be decided again with the confirmation it arrived without, by anyone who saw the signature on the way (§10.5). Neither choice closes the replay §14 names for a set withdrawn before the move; this one does not add a second.

**A signed set on a box past its expiry cannot be taken back.** Question 47, decided 2026-09-15. `DELETE /offers/{id}/decisions` MUST refuse, with `409 not_withdrawable`, a physical offer whose `expires_at` has passed. A withdrawal returns the lines no collection named to `offered`, and on a box past its expiry the deadline then makes them `lost` once the grace has passed, which §3.2 never bills. So a household that signed `kept`, let the box go uncollected, and withdrew inside a cooling window longer than the expiry and the grace kept the goods and settled at nothing, and the loss moved to whoever holds the stock. The rule runs from the expiry and not from the end of the grace, because a withdrawal inside the grace leaves the same lines to go `lost` a little later. **What it costs** is the part of a long window that outlasts the box: a household that wants to take back a set after the expiry can no longer do it, and what it kept is settled as kept. Found by a review of question 46 on 2026-09-15. **The reason given for leaving a digital offer untouched was wrong**, and the third refutation pass over question 68 falsified it: it read "its undecided lines become `returned` at expiry and nothing is kept for nothing", which is true of an ordinary digital offer and false of a ceremonial one, where clause 25's default ships exactly one line when nothing was kept (§12). So a digital gift withdrawn past its expiry is precisely the shape that turns a refusal into a charge, and it was the shape the pass measured. What confines this rule to physical boxes is not that a digital offer is harmless but that **a withdrawal now needs the household's own signature** (the paragraph below), so the party that can reopen a digital set past its expiry is the household itself, deciding again about its own gift. Were that signature not there, this rule would have to reach a ceremonial offer too.

**A signed decision is taken back by a signature over taking it back** (decided 2026-09-20, after the third refutation pass over question 68). `DELETE /offers/{id}/decisions` MUST carry the household's signature, or its passkey's assertion, over the canonical form

```
valence.withdraw.1
<offer id>
<the moment the set was decided>
```

and an implementation MUST refuse `422 bad_signature` where it does not verify against the key registered for the offer's household, and `422 unsigned` where no key is registered. The moment is inside the bytes, so a signature covers the set that stands and not the next one: a re-decision stamps a new moment, and a signature seen on the way is spent. An offer's representation MUST therefore carry `decided_at`, because a member's client cannot sign for a moment it cannot read. **The refusals that are decided before the mandate is read stay before this too**, so a set nobody signed still answers `409 not_withdrawable` rather than being asked to sign away a commitment it never made.

**Why it was not asked for, and what changed.** Withdrawing removes a commitment, and every other rule about signatures in this specification is about adding one; §16.1 recorded the hole in 2026-09-11 and left it for §15's nonce. Question 68 then made the window the longest `cooling_seconds` across every mandate the household holds, and **the window is the interval in which this route is open**, so the change lengthened it. The pass measured the join: a recipient signed `returned` on every line of a ceremonial offer, which owes nothing and was final; a caller holding nothing but the offer id took that back five days later; §12 defaulted the first line at the expiry and the giver was charged 1,200 for a gift the recipient had refused in writing. `origin/main` refuses the same attack, because there the window came from the label the offer named and had closed. **Two facts each checked, and a join nobody checked**: §16.1 names the unauthenticated route, §16.5 names the longer window, and both sections were right. **What it costs** is that a member whose key this host does not hold cannot take a set back at all, which is the member who moved and has not signed a mandate here (§14.2), and that a client must read the offer before it can sign.
...
## Appendix C. Digital quotation archives

A household archive carrying digital carriage quotations uses `valence-node/9` and includes `carriage_quotes` rows naming offer, carriage and quotation time. Quotations **MUST** travel only with their household’s digital offer, preserve the original amount and time, and remain at the source with offers excluded under §14.2. A repeated import **MUST NOT** add or replace a quotation for an offer already carried. Older archives carry no quotation. A host unable to retain a nonempty quotation register **MUST** refuse it. This archive does not transfer deployment-bound operation authorisation.
