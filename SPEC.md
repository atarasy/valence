...
- **A waiting statement stands on the household's own surface, and it is not a reminder.** From the moment a collection records a consumed line until the statement is signed, the hub MUST show that household that a statement is waiting and MUST make it reachable. **The refusal above then falls on a household that has had the thing in front of it**, rather than on one that learns only that a box stopped coming.
...
16. charges a physical box's consumed lines only on the household's signature over the settlement statement, leaves a disputed line off the charge, refuses to settle such a box with no delivery recorded, and presents no further physical box **from that presenter** to a household while a statement stands unsigned (§6.5)
17. refuses a ceremonial offer in the physical binding (§12), and renders on both the approval and the statement the carriage the hub holds, asking the hub for it where it does not hold the register itself (§13.1, §7.5b)
