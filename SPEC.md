...
- **Every confirmation names an offer the import carries.** Question 50, decided 2026-09-15. A register entry for an offer the body does not carry would add a token to one this host already holds, which unlocks the withdrawal §16.5 refuses and lets a captured signature decide the set again. Measured by a refutation pass the same day, which put a token on a held offer with a body carrying nothing else. **An offer and its register arrive together**: an implementation that writes an import's offers before refusing a later row MUST have written each offer's register with it, because the refused retry cannot add it afterwards and the offer would stand as a set moved without one. The reference is not atomic, which this section's first sentence of refusal asks it to be; that is a known defect of the reference and not a licence.
