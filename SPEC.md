...
GET    /_node/mandates?household=  whether this household has any mandate here, as `has` (§16.2, question 56)
```
...
**The ceiling counts per host** (question 63, decided 2026-09-19, accepted as a cost). A household that moves in the middle of a day can settle past its ceiling across the two hosts, because the settlements a move carries are not counted towards the new host's day, and a ceiling tightened at the new host does not reach a set left at the old one (§14.2). Counting the carried settlements was refused: the import authenticates nobody (§13.2), so anybody who can post one could fill a household's day with settlements that never happened.
