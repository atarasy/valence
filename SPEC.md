...
**A gift is presented only on its giver's signature** (question 64, decided 2026-09-19). An implementation MUST refuse to present a ceremonial offer with `422 gift_unsigned` unless the body carries the giver's `signature` or passkey `assertion` over the gift, and with `422 bad_signature` where it does not verify against the key registered under the giver's name, and MUST reserve nothing before it verifies. The bytes are, one to a line, `valence.gift.1`, the offer's id, the giver, the recipient household, the presenter, the band's minimum and maximum, the most the offer can come to (§6.4's upper bound, which leaves out what a maker gave) and the expiry; a host serves them at `GET /offers/{id}/gift` with the passkey challenge beside them, and a giver's device recomputes them from the offer rather than trusting them. Until this question the giver was whatever household the presenter wrote: measured by a refutation pass, a household named as giver was charged 1500 without any act of its own, and after question 60 its own purchases were then refused on its daily ceiling. An offer that is not ceremonial MUST NOT carry the field.
...
**A mandate that arrived by a move is signed here before it is one** (§14.2, question 56, decided 2026-09-18). What a move carries is a claim, and this section is the route by which a claim becomes a mandate: the household signs the claim's own canonical bytes, at the version the claim carries. **Only the assertion shape is bound to a host**, by the relying party inside the authenticator's data; the canonical form names no host, so a signature made for one host records the same claim at another, and whoever relayed a submission once can do that without the person being present. Named by a refutation pass on 2026-09-18 and written here rather than fixed, because binding the first shape to a host is a change to the canonical form and every signature over it. **It is question 58**, opened 2026-09-19. A fourth pass measured its reach as every version a household has ever signed, at any host that holds none of its history, through three routes a version rule cannot close; restoring version 1 moved the exposure and did not shrink it.
