...
**Question 57, decided 2026-09-18: a move carries what happened and not what is in progress.** An implementation MUST refuse, with `409`, an offer that arrives in `drafted` or `presented`, and MUST refuse a candidate that arrives with no verdict. Measured on the reference the day it was decided, an import took an offer at `presented` with a `201` and **no reservation on the receiving ledger**; §6.4 makes the reserve the upper bound of what an offer may settle at, so an offer that can still be decided and has no reserve is one whose upper bound this ledger has never seen. What stopped it charging was `commit` refusing with `no_reservation`, which is the ledger's answer and not this specification's, and another adapter may answer differently. A candidate arriving with no verdict is the same shape one layer down: `decide` reads a valence that is not `offered` as one already decided, so such a candidate could never be decided and its line would settle at nothing.

**The cost is that a proposal in flight is lost by a move**, and the presenter can make it again at the host the household arrived at. The other shape considered was presenting it again on arrival, which would reserve here and run the checks a presentation runs; it was refused because arrival would then change the offer, and a move could fail on a rule about an offer the household never made, §5.1's being the plain case.
...
**A claim is not a signed history, so when one is all a host holds, the version is the household's to state.** An implementation MUST accept any version at such an identifier and MUST require version 1 only where it holds neither a mandate nor a claim. Requiring 1 made a stranger's claim cost the household its version line: it recovered only by starting again. And **a version MUST have room to be followed**: a version at or above 2^53 does not increase when one is added to it, so the version stops rising and an old signature replays onto a new record, which is the one thing §16.1 puts the version in the bytes to prevent.
