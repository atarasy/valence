...
**A version's `lapses_at` is at most a year after it is recorded** (clause 58, decided the same day). An implementation MUST refuse with `422 lapse_too_far` a version whose `lapses_at` is further than 366 days past the moment it records it; a year is counted as the longest calendar year, so that a hub which renews a year out from a member's own clock, as the reference hub does (`Date.now() + 365` days on the device), is not refused over the minutes between two clocks. Question 68 made every mandate a household holds bind every offer it makes, and the paragraph above made its lapse the co-signers' to bring forward, so a mandate naming a co-signer nobody holds (a typo, or a key the person lost) holds its household until it lapses, and nothing bounded that: the same refutation pass recorded a mandate lapsing in the year 9999, whose daily ceiling of 0 then refused everything under every label. **A household that has lost a co-signer is therefore held for at most a year**, and that year is the cost: nothing in this section lets it out sooner, since refusing a co-signer whose key is not registered at record time would not reach one lost afterwards. It is also the cost to every household of renewing at least yearly, which clause 58 already asks of a standing mandate. A claim that arrives by a move with a lapse beyond the bound can never be signed here, and the household records its own version 1 instead (§14.2).
