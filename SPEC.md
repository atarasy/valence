...
- **Every confirmation names an offer the import carries.** Question 50, decided 2026-09-15. A register entry for an offer the body does not carry would add a token to one this host already holds, which unlocks the withdrawal §16.5 refuses and lets a captured signature decide the set again. Measured by a refutation pass the same day, which put a token on a held offer with a body carrying nothing else. A register that is not a map from offer ids to lists of tokens is refused with `400`, because an entry quietly skipped leaves its offer standing as a set moved without one. **The reference import is not atomic**, against this section's rule to refuse the whole import: rows before the one refused stay written, and a retry is refused as already held. The reference writes each offer's register with the offer so that this defect cannot also produce a set moved without one; the rows after the refusal, among them collections and mandates, are still lost, which question 51 records.
