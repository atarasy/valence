...
export type MemberWithdrawalEnvelope = {
  profile: typeof MEMBER_WITHDRAWAL_PROFILE; environment: string; origin: string; rpID: string;
...
export function verifyMemberWithdrawal(e: MemberWithdrawalEnvelope, assertion: Assertion, key: string,
  scope: MemberWithdrawalScope | undefined, rpID: string, canonical: string,
  ...
  try {
    ...
    if (e.profile !== MEMBER_WITHDRAWAL_PROFILE || e.environment !== scope.environment || e.origin !== scope.origin || e.rpID !== rpID) return false;
    ...
    const request = JSON.stringify(['atarasy.member-withdrawal-operation.1', JSON.stringify([1, e.environment, e.origin]), e.principal, e.credential, e.household, e.keyFingerprint, e.offer, e.mandate, e.presenter, e.canonical, e.reviewedRevision, e.expiresAt]);
...
export function memberWithdrawalIdentity(e: MemberWithdrawalEnvelope, a: Assertion): string {
  return digest(JSON.stringify([memberWithdrawalBytes(e).toString(), a.authenticator_data, a.client_data_json, a.signature]));
}
