import pathlib
# Clause 47, §16.2, question 68. Read the out-of-network ceiling of the mandate
# the offer names alone, as before the question: a household that had tightened
# one label then presents under a second it recorded by itself. The giver's
# branch, question 65's, is left as it is.
# Re-anchored when question 68 was rebased onto questions 65 to 67.
p = pathlib.Path('src/engine/offers.ts'); s = p.read_text()
old = """      : mandate
        ? Math.min(
            mandate.ceiling_out_of_network,
            (await this.mandateSource.outOfNetworkCeilingOf(offer.household, now)) ?? Infinity
          )
        : null;
"""
assert s.count(old) == 1, "anchor drifted"
s = s.replace(old, "      : mandate?.ceiling_out_of_network ?? null;\n", 1)
p.write_text(s)
