...
export function connectJournalFixture(dir: string, clock = { at: 1000 }) {
  ...
  // §13.2, question 55. The household is the name of the key this fixture signs
  // with, and its mandate hangs from that identifier.
  const pem = readFileSync(join(dir, 'public.pem'), 'utf8');
  const HOUSE = nameOf(pem), JOURNAL_MANDATE = `${HOUSE}.journal`;
  const { engine } = makeEngine(); engine.registerIdentity(HOUSE, pem);
  ...
  return { authority, login, bindings, journal, clock, HOUSE, JOURNAL_MANDATE, close() { journal.close(); bindings.close(); login.close(); authority.close(); } };
}
export async function seedJournalFixture(dir: string) {
  ...
  const { HOUSE, JOURNAL_MANDATE } = fixture;
  ...
  fixture.authority.bindResource({ kind: 'mandate', id: JOURNAL_MANDATE }, { household: HOUSE });
  ...
  fixture.bindings.bind(session.token, JOURNAL_MANDATE);
  const terms: StatementTerms = { offer: 'journal-offer', mandate: JOURNAL_MANDATE, presenter: 'merchant-1', canonical: 'valence.statement.1\njournal-offer\n550\ncandidate:consumed:1200:', reviewedRevision: 'a'.repeat(64), expiresAt: 4000 };
