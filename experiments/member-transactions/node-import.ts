...
// D-1: `name`/`variant` are catalogue display text, present only when the
// catalogue gave one. A block from before it has no such key, so both are
// optional here exactly as `contact` is above.
const displayText = (max: number): Check => v => { if (typeof v !== 'string' || !v || [...v].length > max) fail(); };
const candidateShape = { id, product: id, quantity: integer, unit_price: integer, merchant: id, maker: id, ships: id, category: nullable(id), predicted_conversion: nullable(v => { if (typeof v !== 'number' || !Number.isFinite(v) || v < 0 || v > 1) fail(); }), is_exploration: bool, given_by: nullable(id), valence, decided_at: nullable(integer), kept_as: nullable(one('self', 'gift', 'order')), lineage: nullable(id) };
const displayExtra = (v: unknown): Record<string, Check> => {
  const extra: Record<string, Check> = {};
  if (v && typeof v === 'object' && !Array.isArray(v)) {
    if ('name' in v) extra.name = displayText(120);
    if ('variant' in v) extra.variant = displayText(60);
  }
  return extra;
};
const candidate: Check = v => object({ ...candidateShape, ...displayExtra(v) })(v);
...
const settlementLineShape = { candidate: id, product: id, merchant: id, maker: id, ships: id, valence, amount: integer, disputed: bool };
const settlementLine: Check = v => object({ ...settlementLineShape, ...displayExtra(v) })(v);
const settlement = object({ offer: id, settled_at: integer, kept_amount: integer, consumed_amount: integer, lost_amount: integer, charged: integer, disputed_amount: integer, lines: array(settlementLine), payer: id, signed_by: id, signed_as: one('agent'), receipt: id, confirmation: nullable(text) });
...
const protectionRecords: Check = value => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail();
  for (const [key, record] of Object.entries(value as Record<string, unknown>)) { id(key); protection(record); }
};
// Question 70: a correction the merchant appended to a signed settlement,
// filed under its offer. The archive check is shape only; the engine's
// import verifies the signature and the ceiling.
const correction = object({ id, offer: id, merchant: id, amount: v => { integer(v); if ((v as number) < 1) fail(); }, kind: one('refund', 'collection'), note: v => { if (typeof v !== 'string' || v.length > 500) fail(); }, corrected_at: integer, signature: text });
const correctionRecords: Check = value => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail();
  for (const [key, list] of Object.entries(value as Record<string, unknown>)) { id(key); array(correction)(list); if (!(list as unknown[]).length) fail(); for (const c of list as { offer: string; id: string }[]) if (c.offer !== key) fail(); unique(list as { id: string }[], c => c.id); }
};
// §6.6a: what became of a refund correction, filed under its offer. Shape
// only, as above; the engine's import verifies the signature and the order.
const correctionReturn = object({ correction: id, offer: id, merchant: id, state: one('returned', 'repaid'), note: v => { if (typeof v !== 'string' || v.length > 500) fail(); }, at: integer, signature: text });
const correctionReturnRecords: Check = value => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail();
  for (const [key, list] of Object.entries(value as Record<string, unknown>)) { id(key); array(correctionReturn)(list); if (!(list as unknown[]).length) fail(); for (const r of list as { offer: string }[]) if (r.offer !== key) fail(); unique(list as { correction: string; state: string }[], r => JSON.stringify([r.correction, r.state])); }
};
const keyMap = (v: unknown) => { if (!v || typeof v !== 'object' || Array.isArray(v)) fail(); for (const [name, pem] of Object.entries(v as Record<string, unknown>)) { id(name); text(pem); } return v as Record<string, string>; };
const schema = object({ keys: keyMap, format: one(EXPORT_FORMAT_VERSION, 'valence-node/12', 'valence-node/11', 'valence-node/10', 'valence-node/9', 'valence-node/8', 'valence-node/7', 'valence-node/6', 'valence-node/5', 'valence-node/4'), carriage_quotes: array(object({ offer: id, carriage: integer, quoted_at: integer })), decided_protections: protectionRecords, corrections: correctionRecords, correction_returns: correctionReturnRecords, household: id, exported_at: integer, offers: array(offer), settlements: array(settlement), notes: array(object({ candidate: id, author: id, text, shared_with: array(one('recipient', 'merchant')), created_at: integer })), lineage: array(object({ id, from: id, to: id, product: id, merchant: id, maker: id, kind: one('gift', 'return', 'regift', 'thanks'), occasion: text, receipt: id, signature: text, attested: bool, created_at: integer })), receipts: array(object({ ref: id, at: integer })), payments: array(object({ offer: id, presenter: id, settled_at: integer, charged: integer, receipt: id })), gifts_in_flight: array(id), recoveries: array(object({ id, household: id, initiated_by: id, at: integer, notified: array(object({ channel: id, controlled_by_recoverer: bool })) })), collections: array(object({ offer: id, due_at: integer, grace_days: integer, collected_at: nullable(integer), returned: array(id), consumed: array(id), missing: array(id), missing_notes: v => { if (!v || typeof v !== 'object' || Array.isArray(v)) fail(); for (const note of Object.values(v as Record<string, unknown>)) { if (typeof note !== 'string' || !note.trim() || note.length > 500) fail(); } } })), permissions: array(object({ id, kind: one('party', 'computation'), result_form: nullable(one('aggregate')), grantee: id, scope: array(id), purpose: text, granted_at: integer, expires_at: integer, asked_from: id, revoked_at: nullable(integer) })), queries: array(object({ id, asked_by: id, product: id, answered: bool, at: integer })), mandates: array(object({ id, household: id, ceiling_out_of_network: integer, co_signers: array(id), ceiling_daily: nullable(integer), cooling_seconds: nullable(integer), lapses_at: integer, version: integer })), deliveries: array(object({ offer: id, carriage: integer, code: text, status: one('placed', 'in_transit', 'delivered', 'returned'), updated_at: integer })), confirmations: v => { if (!v || typeof v !== 'object' || Array.isArray(v)) fail(); for (const [k, values] of Object.entries(v!)) { id(k); array(id)(values); unique(values as string[], x => x); } } });
...
export function validateNodeImport(input: unknown, household: string): NodeExport {
  ...
  if (node && ['valence-node/4', 'valence-node/5', 'valence-node/6', 'valence-node/7', 'valence-node/8'].includes(node.format) && !Object.hasOwn(node, 'carriage_quotes')) node.carriage_quotes = [];
  if (node && ['valence-node/4', 'valence-node/5', 'valence-node/6', 'valence-node/7', 'valence-node/8', 'valence-node/9'].includes(node.format) && !Object.hasOwn(node, 'corrections')) node.corrections = {};
  // §14.2. A /10 archive and earlier carry no keys: its edges verify with what the receiving host already holds.
  if (node && node.format !== EXPORT_FORMAT_VERSION && node.format !== 'valence-node/12' && node.format !== 'valence-node/11' && !Object.hasOwn(node, 'keys')) node.keys = {};
  // §6.6a. Before /12 no archive carries a record that a refund came back.
  if (node && node.format !== EXPORT_FORMAT_VERSION && node.format !== 'valence-node/12' && !Object.hasOwn(node, 'correction_returns')) node.correction_returns = {};
  ...
  for (const rows of [node.settlements, node.collections, node.deliveries, node.carriage_quotes]) unique(rows as { offer: string }[], x => x.offer);
  ...
  for (const key of Object.keys(node.corrections)) if (!offers.has(key)) fail();
  for (const [key, list] of Object.entries(node.correction_returns)) { if (!offers.has(key)) fail(); for (const r of list) if (!(node.corrections[key] ?? []).some(c => c.id === r.correction)) fail(); }
  ...
  for (const q of node.carriage_quotes) if (offers.get(q.offer)?.binding !== 'digital') fail();
