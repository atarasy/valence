...
export function validateNodeImport(input: unknown, household: string): NodeExport {
  ...
  if (node && node.format !== EXPORT_FORMAT_VERSION && node.format !== 'valence-node/11' && !Object.hasOwn(node, 'keys')) node.keys = {};
  // §6.6a. Before /12 no archive carries a record that a refund came back.
  if (node && node.format !== EXPORT_FORMAT_VERSION && !Object.hasOwn(node, 'correction_returns')) node.correction_returns = {};
