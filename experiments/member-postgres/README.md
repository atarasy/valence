...
Run deployment/build.ts with an explicit new isolated output directory (existing output is refused) to bundle the API and write its Vercel configuration. It builds the development target unless given `--target production`. Vercel uses Bun 1.x, Singapore and a 60-second function limit. The entry integrates the pg pool lifecycle, rejects other origins, takes the trusted peer from x-vercel-forwarded-for, and keeps generic errors free of connection details, logging only a fixed, safe reason when startup or a request fails (`deployment/serve.ts`'s `failureReason`). Documented ingress behaviour: https://vercel.com/docs/headers/request-headers. Requests bypassing Vercel are not supported by this entry. The schema/bootstrap command is separate from the deployed bundle and verifies the exact dedicated Neon project ID. Before deploying, compare config.ts/deployment/config.json with the last deployed commit; if the runtime profile changed, rebind the database with `rebind-runtime.ts` first (see MIGRATION_COMMAND.md), then promote.
...
[Production](deployment/PRODUCTION.md) is a second target of the same code at `https://members.vox.delivery`, in its own Vercel project and its own Neon project. `deployment/targets.ts` holds both targets; `build.ts` and `bootstrap.ts` take `--target production`; `member-invite.ts` issues the single-use App Review invitation, and the reviewer's own steps do the rest: the first sign-in adopts the household and grants the review shop, and signing the version-1 mandate presents one digital proposal in the same request (`review-proposal.ts` is the operator fallback). The production entry also serves `/privacy` and `/support`. **Production and development share no data, credentials or passkeys**: the Neon projects, the deployment ids and the relying parties all differ. Every command that connects checks the Neon project id and then refuses a database that already holds the other target's deployment.

## Adoption at the first sign-in and the invitation lifetime, 2026-09-23

Two founder decisions, in every environment. **A household is adopted at the first sign-in whose assertion verifies** (`member-adoption.ts`, called by `login.finish` before the session is created): the principal takes the household its credential's key names, the key is registered under that name, and the version-1 mandate is written as a claim for the device to sign. Registration alone adopts nothing, a household another live principal holds is refused, and a later sign-in changes nothing. The claim's defaults and the reason for each are in [PRODUCTION.md](deployment/PRODUCTION.md). **Invitations live `invitationLifetimeMs`, fourteen days**, a configuration value of their own; ceremonies and operations keep `maximumLifetimeMs`. That changed the runtime profile to `atarasy.member-runtime.3`, so **the development deployment needs `rebind-runtime.ts` run immediately before its next promotion**, or it answers 503 on every route; the exact order is in PRODUCTION.md.
