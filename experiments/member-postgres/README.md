...
**The one that outlives this package is the mandate the operator writes.** `Mandates.importMandate` checks no signature, no name and no version, and the household is now the real name of a real device key, so the acceptance flow mints a version 1 that key's holder never signed and can only build a version 2 on top of. The §14 node import route calls the same function, which is question 56's open half. The terms written here are now the narrowest the acceptance box needs, one box's price and carriage and a week, rather than the widest the shape allows; **that is containment and not a fix**. The fix is for the device to record its own mandate with an assertion, which §16.1's `record` already accepts and this service has no route for.
