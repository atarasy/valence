...
/**
 * This store's own refusals, so a command can tell them from the driver's. A
 * deployment fenced by its control row printed as "unknown", indistinguishable
 * from a bad password, because the error had no class. Measured by a seventh
 * refutation pass on 2026-09-16.
 */
export class StoreError extends Error {}
export function storeSavepoint<T>(store:Store,fn:()=>T):T {const save=savepoints.get(store);if(!save)throw new StoreError('Unknown PostgreSQL capability');return save(fn);}
...
export async function initialiseDeployment(pool:Pool,input:Identity) {
 ...
 try {
  ...
  const r=rows[0];if(!r||r.environment!==p.environment||r.origin!==p.origin||r.epoch!==p.epoch||!r.enabled)throw new StoreError('PostgreSQL writer fenced');
...
export function postgresStore(pool:Pool,input:Identity) {
 ...
 return {
  async run<T>(work:(store:Store)=>Promise<T>|T):Promise<T> {
   ...
   const check=()=>{if(!active)throw new StoreError('PostgreSQL scope ended');};
   try {
    await c.query('BEGIN');
    ...
    if(!control||control.environment!==p.environment||control.origin!==p.origin||control.epoch!==p.epoch||!control.enabled)throw new StoreError('PostgreSQL writer fenced');
    ...
    if(bounds.rows[0].n>10000||Number(bounds.rows[0].bytes)>16_777_216)throw new StoreError('PostgreSQL snapshot limit');
    ...
    const store:Store={
     map<V>(namespace:string):Map<string,V> {
      check();if(!validName(namespace)||used.has(namespace))throw new StoreError('Invalid or duplicate PostgreSQL map');used.add(namespace);
      ...
      proxy=new Proxy(map,{get(target,property){
       check();
       if(property==='set')return (key:string,value:V)=>{check();const encoded=JSON.stringify(value);if(typeof key!=='string'||encoded===undefined||Buffer.byteLength(encoded)>1_048_576)throw new StoreError('Invalid PostgreSQL value');if(operations.length>=10000)throw new StoreError('PostgreSQL write limit');operations.push({kind:'set',namespace,key,value:encoded});target.set(key,value);return proxy;};
       if(property==='delete')return (key:string)=>{check();if(typeof key!=='string')throw new StoreError('Invalid key');if(operations.length>=10000)throw new StoreError('PostgreSQL write limit');operations.push({kind:'delete',namespace,key});return target.delete(key);};
       if(property==='clear')return ()=>{check();if(operations.length>=10000)throw new StoreError('PostgreSQL write limit');operations.push({kind:'clear',namespace});target.clear();};
       ...
       throw new StoreError('Unsupported PostgreSQL map access');
      }});return proxy;
     },close(){throw new StoreError('PostgreSQL transaction owns store');}
    };
    savepoints.set(store,<R>(fn:()=>R):R=>{
     check();if(fn.constructor.name==='AsyncFunction')throw new StoreError('Savepoints require synchronous work');
     const mark=operations.length,backups=liveMaps.map(map=>structuredClone([...map]));
     try{const value=fn();if(value&&typeof (value as any).then==='function')throw new StoreError('Savepoints require synchronous work');return value;}
    ...
    if(after.rows[0].n>10000||Number(after.rows[0].bytes)>16_777_216)throw new StoreError('PostgreSQL snapshot limit');
