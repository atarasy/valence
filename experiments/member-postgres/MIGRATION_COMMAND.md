...
| `presenter_correction_body_unchecked` | Question 70. The presenter surface's own unknown-field refusal for a posted correction | SURVIVED, and correctly so: unlike `delivery` and `carriage-quote`, the corrections route forwards `input` unchanged rather than reconstructing the body from named fields, and `engine/src/http.ts`'s own `strict()` call refuses the identical allowed-key set (`id, merchant, amount, kind, note, corrected_at, signature`) before the body is ever written. Removing the presenter-layer copy is unobservable at the HTTP boundary; it is kept anyway, for the earlier refusal and so the engine's own error wording is not the first thing a shop sees, but no HTTP-level probe can distinguish the two layers, so no probe is claimed for this one |
