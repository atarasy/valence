...
const keys='androidAppOrigins,bodyTimeoutMs,budgetWindowMs,dayBoundary,environment,explorationRate,maxSessionLifetimeMs,maximumBodyBytes,maximumLifetimeMs,maximumPending,maximumRequests,maximumTrackedTokens,origin,recoveryGraceDays,reminderLimit,rpID';
export function memberRuntimeIdentity(input:MemberRuntimeConfig) {
 ...
 return {profile:'atarasy.member-runtime.2' as const,config:Object.freeze(c),fingerprint:createHash('sha256').update(JSON.stringify(['atarasy.member-runtime.2',ordered])).digest('hex')};
}
