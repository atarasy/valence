...
const [action,presenter,presenterKeyPath,merchant,merchantKeyPath,output,...extra]=process.argv.slice(2);
const paths=[presenterKeyPath,merchantKeyPath,output];
if(action!=='issue'||extra.length||!presenter||!merchant||paths.some(p=>!p||!isAbsolute(p)))throw new Error('Usage: presenter-credential.ts issue <presenter> /abs/presenter.pem <merchant> /abs/merchant.pem /abs/private/new-credential.json');
...
try{
 ...
 try{
  const issued=await unit.run(s=>registerPresenter(memberRuntime(s,c),{presenter:presenter!,presenterKey,merchant:merchant!,merchantKey,at:Date.now()}));
