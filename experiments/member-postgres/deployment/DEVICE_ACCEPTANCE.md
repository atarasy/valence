...
`statement` runs once, after the device has registered exactly one passkey. In one locked unit it registers a development presenter and merchant with ephemeral Ed25519 keys whose private halves are dropped, a signed catalogue with one physical product at 1,200 and a signed disclosure saying no goods ship and no payment is taken. It imports a mandate for the acceptance household whose key is the registered passkey's own P-256 public key, creates and presents one physical box, records a delivery with carriage 550 and a collection marking the goods consumed. It grants the presenter, binds mandate and offer ownership, and binds the mandate to that credential.
