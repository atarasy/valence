...
**The mandate this step writes is not signed by anybody.** It is written with `importMandate`, which checks no signature, no name and no version, and the household is the real name of the device's real key, so the holder of that passkey can afterwards only record version 2 on top of terms they never agreed to. The terms are therefore the narrowest the box needs: one box's price and carriage as the out-of-network ceiling, no daily ceiling, no cooling window, no co-signers, and a week. **Do not widen them**, and do not run this against a device whose passkey is or will be a real member's.
