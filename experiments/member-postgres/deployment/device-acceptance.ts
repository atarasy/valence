...
if(!['prepare','status','invite','statement'].includes(action??'')||extra.length||(action==='invite'?(!output||!isAbsolute(output)):output!==undefined))throw new Error('Usage: device-acceptance.ts prepare | status | statement | invite /absolute/private/new-file.json');
