...
export function openStatementAuthorisations(base: ReturnType<typeof memberRuntime>, options: Policy) {
  ...
  const scope = { environment: policy.environment, audience: policy.origin };
  ...
  function run<T>(work:(r:ReturnType<typeof runtime>)=>Promise<T>|T) { return Promise.resolve(work(runtime())); }
  function runtime() {
    ...
    return { engine, authority, login, bindings, journal, reviews,operations,snapshot,saved,response };
  }
  async function verifyInUnit(r: ReturnType<typeof runtime>, token: string, operation: JournalOperation, review: ReturnType<ReturnType<typeof runtime>['saved']>, fixed: PreparedAssertion) {
    ...
    r.reviews.put(operation.id,review);
  }
  ...
  return {
    ...
    prepare(token: string, input: { offer: string; disputed: string[] }) {
      ...
      return run(async r => {
        ...
        r.reviews.insert(operation.id,review);
        return r.response(operation, review);
      });
    },
